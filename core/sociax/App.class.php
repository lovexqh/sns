...
{//类定义开始
    ...
    {
        global $ts;
        //2013/07/13小波修改支持自动判断是否大桌面调用并调用相应组的模板
        if (empty($_SERVER['system']) || $_SESSION['system']=='desktop' ) {
            $ts['system'] = 'desktop';
        } 
    ...
    static private function checkTemplate()
    {
        ...
        //[模版名称]2013/05/13小波修改支持自动判断是否大桌面调用并调用相应组的模板 start
        $blacklist 	= C('NOT_DESKTOP_THEME');
        //加入白名单支持
        if (!in_array(strtolower(APP_NAME), $blacklist) && !empty($ts['system']) && $ts['system'] == 'desktop') {
            define('TEMPLATE_NAME', 'desktop');
        }else{
            define('TEMPLATE_NAME', C('DEFAULT_THEME'));
        }
        //[模版名称]2013/05/13小波修改支持自动判断是否大桌面调用并调用相应组的模板 end
    ...
    static private function checkUser() {
        ...
        if (!service('Passport')->isLogged()) { // 未登录
            ...
            if (APP_NAME == 'home' && MODULE_NAME == 'Space' && !$ts['site']['site_anonymous']){
                //$_SESSION['refer_url'] = defined('SITE_URL')?SITE_URL . $_SERVER['REQUEST_URI']:'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
    ...
    static public function execApi(){
        include_once (SITE_PATH.'/api/mobileApi/'.MODULE_NAME.'Api.class.php');
        $class = MODULE_NAME.'Api';
        $module = new $class;
        $action = ACTION_NAME;
        //执行当前操作
        if($_REQUEST['format']=='html' && !headers_sent() && extension_loaded("zlib") && strstr($_SERVER["HTTP_ACCEPT_ENCODING"],"gzip")){
            ob_start('ob_gzip');
            call_user_func(array(&$module,$action));
            ob_end_flush();
        }else{
        ...
        //添加jsonp形式 徐程亮 13-7-3 上午9:44
        $format = (in_array( $_REQUEST['format'] ,array('xml','json','php','test','jsonp','html','text') ) ) ?$_REQUEST['format']:'json';
        if($format=='json'){
