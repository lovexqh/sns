 ...
 *          +------------------------------------------------------------------------------
 */
abstract class Action extends Think { // 类定义开始
	...
	public function __construct() {
		...
		/*
		if(!empty($_GET['app']) && $_GET['app'] != 'square' && $_GET['app'] != 'tv' && $_GET['app'] != 'blog' && $_GET['app'] != 'video'){
			//在应用中打开新页面 孙晓波添加于2013-8-6
			if(!empty($_GET['show_code'])&&$_GET['show_code']==md5("gridsoft")&&!empty($_GET['app'])){
				$map['app_name'] = $_GET['app'];
				$app = M('app')->where($map)->find();
				if(false!==strpos($_SERVER['REQUEST_URI'],'#')){
					$temp_url = substr($_SERVER['REQUEST_URI'], 0, -43);
				}else{
					list($temp_url,$auth) = explode("#", $_SERVER['REQUEST_URI']);
					$temp_url = substr($_SERVER['REQUEST_URI'], 0, -43);
					$temp_url .= $auth;
				}
				//$temp_url = str_ireplace('&anchor=', "#", $temp_url);
				if (!strpos($temp_url,'iframe=')) {
					$temp_url .= (strpos($temp_url,'?')?"&":"?")."iframe=yes";
				}
				//验证是否有标题
				if(!empty($_GET['show_title'])){
					$param['title'] = $_GET['show_title'];
				}
				$param['redirect'] = base64_encode($temp_url);
				$url = U($_GET['app'].'/'.$app['entry'],$param);
				//echo $url;
				//exit();
				redirect($url);
			}
		...
		*/
		// 授权检查 孙晓波于2013-7-8 start
		global $ts;
		if (! S ( 'license' )) { // 如果没有license缓存则创建
			S ( 'license', array () );
		}
		$licensewhitelist = array (
				'admin',
				'home' 
		);
		$license = S ( 'license' ); // 从缓存中获取license值
		$appName = $ts ['_app'];
		
		if (0) { // 如果license有问题则跳转
			redirect ( U ( 'home/Public/errorLicense' ) );
		}
		// 授权检查 孙晓波于2013-7-8 end
	...
	protected function initUser() {
		...
		if ($this->mid > 0 && (empty ( $_SESSION ['userInfo'] ) || ! $_SESSION ['userInfo'] ['is_init'] || ! $_SESSION ['userInfo'] ['is_active'])) {
			...
			D ( 'User', 'home' )->setUserObjectCache ( array (
					$_SESSION ['userInfo'] 
			) );
		}
		...
		if (UC_SYNC) {
			...
			// 获取社区用户与UC用户相关联的ID
			$uc_uid = M ( 'ucenter_user_link' )->where ( "uid = '" . $userInfo ['uid'] . "'" )->getField ( "uc_uid" );
			unset ( $map );
			$ucinfo = get_baseinfo_by_uid ( $uc_uid );
			// 缓存UCenter的用户资料
			$_SESSION ['ucInfo'] = $ucinfo;
			$this->assign ( 'ucInfoCache', $ucinfo );
			// array_push($userInfo,$ucinfo);
		}
		...
		// 获取本人操作权限
		$userAppLimit = get_userAppLimit_by_uid ( $this->mid );	
		
		// 重新格式化应用权限下标
		foreach ( $userAppLimit as $applimit ) {
			// 将权限去转数组去重复
			$applimit ['rolearray'] = array_unique ( explode ( ',', $applimit ['roleextend'] ) );
			$ts ['userAppLimit'] [$applimit ['app_name']] = $applimit;
		}
		
		$this->prefix = C ( 'DB_PREFIX' );
		foreach ( $ts ['userAppLimit'] as $key => $value ) {
			$count = explode ( '_', $value [app_name] );
			if (count ( $count ) >= 2) {
				$ts ['userAppLimit'] [$key] ['cid'] = M ( '' )->table ( $this->prefix . 'dsk_clink' )->where ( "url like '%" . $count [0] . "%' and url like '%" . $count [1] . "%'" )->getField ( 'cid' );
			} else {
				$ts ['userAppLimit'] [$key] ['appid'] = M ( '' )->table ( $this->prefix . 'dsk_apps D' )->join ( $this->prefix . 'app A ON A.app_id=D.oid' )->where ( "A.app_name='$value[app_name]'" )->getField ( 'D.appid' );
			}
		}
		...
		if (! $ts ['user'] ['is_init'] && ! canAccess ()) {
			$this->assign ( 'jumpUrl', U ( 'home/Public/checkRegister' ) );
			$this->error ( '请先完善个人资料' );
			exit ();
		}
	}
	protected function initUserApp() {
		...
		$sys_apps = model ( 'App' )->getSystemApp (); // 获取系统应用
		
		$ts ['system_app'] = $system_app; // 系统应用列表 2012/00/6
		...
		if (! $has_installed && model ( 'App' )->isAppAdmin ( APP_NAME, $ts ['_mod'] ))
			$has_installed = true;
		if (! $has_installed) {
			$this->assign ( 'jumpUrl', U ( 'home/Index/install', array (
					'app_name' => APP_NAME,
					'app_id' => 0 
			) ) );
		...
		if ($ts ['site'] ['my_status']) {
			...
			foreach ( $default_app as $v ) {
				if (in_array ( $v ['appid'], $ids ))
					continue;
				$ids [] = $v ['appid'];
				$user_app ['myop_default_app'] [] = array (
						'app_id' => $v ['appid'],
						'app_alias' => $v ['appname'],
						'display_order' => $v ['displayorder'] 
				);
		...
		if (is_array ( $groups )) {
			...
			$group = "FIND_IN_SET('$groups',group_id)";
		}
		
		$myapps = model ( 'App' )->field ( 'app_id' )->where ( $group )->findAll ();
		unset ( $result );
		arrayFormat ( $myapps, $result );
		
		// 处理展示的应用
		foreach ( $user_app as $key => $value ) {
			$_result = array ();
			foreach ( $value as $_k => $_v ) {
				if (is_array ( $result ) && in_array ( $_v ['app_id'], $result ))
					$_result [] = $_v;
			}
			$_user_app [$key] = $_result;
		}
		
		$ts ['user_app'] = $_user_app;
	}
	...
	protected function getUserGroupIds() {
		...
		foreach ( $roles as $val ) {
			if (empty ( $role )) {
				$role = "FIND_IN_SET('$val',role_private)";
			} else {
				$role .= " or FIND_IN_SET('$val',role_private)";
			}
		}
		if (empty ( $roles ))
			return 1;
		else {
			$mygroup = model ( 'AppGoup' )->field ( 'group_id' )->where ( $role )->findAll ();
			unset ( $result );
			arrayFormat ( $mygroup, $result );
			return $result;
		}
	}
	...
	/**
	 ...
	 +----------------------------------------------------------
	 + 创建时间：	2013-10-11 上午10:57:20
	 +----------------------------------------------------------
	 */
	protected function initSafeRequest(){
        @set_magic_quotes_runtime(0);   
        // 判断 magic_quotes_gpc 状态   
        if (@get_magic_quotes_gpc()) {   
            $_GET = stripslashes_array($_GET,true);   
            $_POST = stripslashes_array($_POST);   
            $_COOKIE = stripslashes_array($_COOKIE);
        }else{
            $_GET = addslashes_array($_GET,true);
   ...
   /**
	...
	protected function ajaxReturn($data, $info = '', $status = 1, $type = '') {
		if (empty ( $data ) && ! empty ( $this->ajaxData ))
			$data = $this->ajaxData;
			// 保证AJAX返回后也能保存文章
		if (C ( 'LOG_RECORD' ))
			Log::save ();
	...
	private function _dispatch_jump($message, $status = 1, $ajax = false) {
		...
		if ($status) { // 发送成功信息
			...
			// [模版名称]2013/06/26小波修改支持自动判断是否大桌面调用并调用相应组的模板 start
				// $this->display(THEME_PATH.'&success');
			$this->display ( THEME_PATH . '/desktop&success' );
			// [模版名称]2013/06/26小波修改支持自动判断是否大桌面调用并调用相应组的模板 end
		} else {
			// 发生错误时候默认停留3秒
			if (! $this->get ( 'waitSecond' ))
