...
class View extends Think
{
    ...
    public function fetch($templateFile='',$charset='',$contentType='text/html',$display=false)
    {
		...
		/*
		//2013/05/13小波修改支持自动判断是否大桌面调用并调用相应组的模板 start
		if (!empty($_SESSION['system']) && $_SESSION['system'] == 'desktop') {
			$blacklist 	= array('home', 'square', 'admin');	//不使用desktop的应用
			//加入白名单支持
			if(!in_array(strtolower(APP_NAME), $blacklist)){
				$templateFile = str_replace('/Tpl/default', '/Tpl/desktop', $templateFile);
			}
		}
		//2013/05/13小波修改支持自动判断是否大桌面调用并调用相应组的模板 end
		*/
    ...
    private function showTrace(){
        ...
        $log    =   Log::$log;
        $this->trace('文章记录',count($log)?count($log).'条文章<br/>'.implode('<br/>',$log):'无文章记录');
