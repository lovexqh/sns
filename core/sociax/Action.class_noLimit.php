   ...
   /**
	...
	protected function initUser()
	{
		...
		foreach($ts['userAppLimit'] as $key=>$value){
			...
			if(count($count)>=2){
				$ts['userAppLimit'][$key]['cid'] = M('')->table( $this->prefix . 'dsk_clink' )
						->where( "url like '%".$count[0]."%' and url like '%".$count[1]."%'" )
						->getField('cid');
			}else{
				$ts['userAppLimit'][$key]['appid'] = M('')->table( $this->prefix . 'dsk_apps D' )
						->join( $this->prefix . 'app A ON A.app_id=D.oid' )
						->where( "A.app_name='$value[app_name]'" )
						->getField('D.appid');
			}
		}
