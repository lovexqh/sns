...
class XattachService extends Service {
	...
	public function upload($attach_type='attach',$input_options=array()) {
		...
		$default_options['save_path']	=	UPLOAD_PATH.'/'.APP_NAME.'/'.$default_options['custom_path'];
			...
			if($options['save_to_db']){
				foreach($upload_info as $u){
					...
					if(!empty($options['save_path'])){
						$map['savepath']	=	str_replace(UPLOAD_PATH.'/',"",$options['save_path']);
					}else{
						$map['savepath']	=	APP_NAME.'/'.$options['save_path'];
					}
				...
				foreach($upload_info as $u){
					...
					$map['extension']	=	strtolower($u['extension']);
					$map['hash']		=	$u['hash'];
					$map['savepath']	=	APP_NAME.'/'.$options['custom_path'];
					$map['savename']	=	$u['savename'];
					$map['uploadTime']	=	time();
					//$map['savedomain']=	C('ATTACH_SAVE_DOMAIN'); //如果做分布式存储，需要写方法来分配附件的服务器domain
					$map['key']	=	$u['key'];
	...
	public function addFile($attach_type='attach',$attach_file) {
		...
		$options['save_path']	=	UPLOAD_PATH.'/'.(defined(APP_NAME)?APP_NAME.'/':'').$options['custom_path'];
	...
	public function getAttach($attachId,$field="*") {
		...
		$map['id']	=	array('in', array_map( 'intval' , $attachIds  ) );
