...
class PassportService extends Service
{
	 ...
	 */
	public function getLocalUser($identifier, $password = null,$is_remember_me = false)
	{
		if (empty($identifier))
			return false;
		...
		if($this->isValidEmail($identifier)){
			...
			$identifier_type = 'uid';
		}elseif($this->isValidSfzh($identifier)){
			$identifier_type = 'sfzh';
		}elseif($this->isValidMobile($identifier)){
		...
		if (!$user) {
			...
			$this->setUserCookie($user['uid'],$is_remember_me);
			return $user;
		}
	}
	...
	{
		$user = $this->getLocalUser($identifier, $password,$is_remember_me);
		//print_r($user);
		//exit();
	...
	public function registerLogin(array $user, $is_remeber_me = false,$password = null)
	{
		...
		if($user && UC_SYNC){
			...
			(empty($user['login_from_dz']))?'':uc_user_synlogin($uc_user['uc_uid']);
		}
	...
	private function _recordLogin($uid,$uname, $is_remeber_me = false){
		...
		if (!$this->getCookieUid()) {
			$this->setUserCookie($uid,$is_remeber_me);
			// 登录积分
	...
	public function ucLogin($username,$password) {
		if(isValidEmail($username)){
			...
			if(UC_SYNC && $user['uid']){
				...
				$uc_user = uc_user_login($username,$password,2);
				if($uc_user[0]>0){
					// 注册
					if($this->isValidEmail($uc_user['3'])){
				...
				if($uc_user[0]>0){
					...
					if(!$uc_user_ref){
						...
						if(md5($password) != $user['password']){
							...
							/*
							 * 这个地方原来是从UIA那边同步密码，现在修改成不同步了，直接返回false
							 */
							return false;
						}
						//登录到系统
        ...
        $result['login'] = '';
		if($user){
			if(UC_SYNC && $uc_user[0]>0){
	        	$result['login'] = uc_user_synlogin($uc_user[0]);
	        }
		}
        $result['user'] = $user;
	...
	public function isValidSfzh($sfzh,$ishave=false){
		if(preg_match('/\d{17}[0-9Xx]/', $sfzh) && (strlen($sfzh) == 18 || strlen($sfzh) == 15 )){
			if(UC_SYNC){
				$res = get_all_by_sfzh($sfzh);
				if($res){
					if(!$ishave){
						return true;
					}else{
	...
	/**
	 * @Title: setUserCookie
	 * @Description: 设置其他应用的cookie信息
	 * @param @param unknown $uid
	 * @return return_type
	 * @author RickerYu lhyfe@sohu.com
	 */
	public function setUserCookie($uid,$is_remeber_me){
		/**
		 * 2013-8-26
		 * Rickeryu
		 * 下面是为了得到uc_uid，这样可以在uia直接获取
		 */
		...
		*/
		...
		//setcookie('LOGGED_USER',jiami("thinksns.{$uid}.{$uc_uid}"),$expire,'/',C('COOKIE_DOMAIN'));
		//setcookie('LOGGED_UIA_USER',jiami("thinksns.{$uid}.{$uc_uid}"),$expire,'/',C('COOKIE_DOMAIN'));
		cookie('LOGGED_USER', jiami("thinksns.{$uid}.{$uc_uid}"), $expire);
		cookie('LOGGED_UIA_USER', jiami("thinksns.{$uid}.{$uc_uid}"), $expire);
	}
}