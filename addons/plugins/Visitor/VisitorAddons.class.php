...
class VisitorAddons extends SimpleAddons
{
	...
	public function getHooksInfo(){
		...
		$this->apply("home_index_right_visitor","home_index_right_visitor");
	...
	// 首页 - 最近来访的人
	public function home_index_right_visitor()
	{
	    $config= model('AddonData')->lget('visitor');
	    if(in_array('home',$config['open'])){
			$data = $this->__getVisitorData($this->mid,$config);
	        $this->assign($data);
	...
	private function __getVisitorData($uid,$config)
	{
		...
		$daycount = M('user_visited')->where("fid={$uid} AND `ctime` >= unix_timestamp('$start') AND `ctime` <= unix_timestamp('$end')")->count();
		$allcount = M('user_visited')->where("fid={$uid} AND ctime>0")->count();

		$data['visitor_title'] = '最近来访的人';
		$data['visitor_list']  = M('user_visited')->field('uid,cTime')
						->where("fid={$uid} AND ctime>0")
						->order('ctime DESC')->limit($config['limit'])->findAll();
		$data['day_count'] = $daycount['count'];
		$data['all_count'] = $allcount['count'];
		return $data;
	}
