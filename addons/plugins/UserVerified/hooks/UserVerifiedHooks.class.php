...
class UserVerifiedHooks extends Hooks
{
	...
	public function home_account_show()
	{
		...
		$memberinfo = get_user_by_uid($uc_uid);
		if( $memberinfo[0] && ( $memberinfo[0]['identitytype']==2 || $memberinfo[0]['identitytype']==3 ) ){
			$identityid = $memberinfo[0]['identityid'];
			$identitytype = $memberinfo[0]['identitytype'];
			if( $identitytype == 2 ){
				$info['identity'] = '老师';
				$userinfo = get_teacher_baseinfo_by_param(array('identityId'=>$identityid));
			}else if( $identitytype == 3 ){
				$info['identity'] = '学生';
				$userinfo = get_student_baseinfo_by_param(array('identityId'=>$identityid));
			}
			$school = getSchoolById($userinfo['schoolid']);
			$info['school'] = $school[0]['xxmc'];
			$info['name'] = $userinfo['xm'];
			$this->assign('info',$info);
		}else{
			$schoolList = getAllSchool();
...
//     	$this->assign('verified', $verified);
	...
	public function home_account_do($param)
	{
		...
		$data['xm'] = preg_match('/^[\x{2e80}-\x{9fff}]+$|^[a-zA-Z\.·]+$/u', $_POST['realname']) ? $_POST['realname'] : '';
		$data['sfzjh']	  = $_POST['idcard'];
		 
		if (!$identity || !$data['schoolId'] || !$data['xm'] || !$data['sfzjh']) {
			echo 0;
		}
		if($identity == 3){
			$result = get_student_baseinfo_by_param($data);
		}else if($identity == 2){
			$result = get_teacher_baseinfo_by_param($data);
		}
		if($result){	//验证通过
			$uc_uid = M('ucenter_user_link')->where('uid = '.$this->mid)->getField('uc_uid');
			$memberinfo = get_user_by_uid($uc_uid);
			if( $memberinfo[0]['identitytype'] ==2 || $memberinfo[0]['identitytype'] == 3 ){
				$this->error('信息已认证');
			}
			
			$identityid = $result['identityid'];
			$findRes = get_members_by_param( array('identityId'=>$identityid, 'identityType'=>$identity) );
			if(!$findRes){
				$res = updateMember($uc_uid, $identityid, $identity);
			}else{
				$this->error('信息已认证');
			}
		}
