 ...
 */
class LSWeiboMedal extends BaseMedal {
	...
	public function getMedalStatus($uid, $medal_id, $user_medal_data, $medal_data) {
		...
		if ( empty($user_medal_data) ) {
			...
			if ( intval($user_medal_data['data']['received_time']) <= 0 && $this->__isLSWeibo($uid) ) {
	 ...
	 * @return bool
	 */
	private function __isLSWeibo($uid, $days = 30) {
		if ($days <= 0) {
			return true;
		}
