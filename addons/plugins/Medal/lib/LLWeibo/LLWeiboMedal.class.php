 ...
 * "微广播达人"勋章
 *
 * 连续3天发原创微广播获得此勋章. 用户获得此勋章即为永久拥有.
 *
 * @author daniel <desheng.young@gmail.com>
 */
class LLWeiboMedal extends BaseMedal {
	...
	public function getMedalStatus($uid, $medal_id, $user_medal_data, $medal_data) {
		...
		if ( empty($user_medal_data) ) {
			...
			if ( intval($user_medal_data['data']['received_time']) <= 0 && $this->__isLLWeibo($uid) ) {
	 ...
	 * @return bool
	 */
	private function __isLLWeibo($uid, $days = 200) {
		if ($days <= 0) {
			return true;
		}
