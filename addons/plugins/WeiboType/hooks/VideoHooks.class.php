...
class VideoHooks extends AbstractWeiboTypeHooks
{
    ...
    public function _weiboTypePublish($type_data)
    {
        ...
        if(!isset($flashinfo)){
        	...
        	$tempparseLink = parse_url($config['video_server_url']);
	        //if(preg_match("/(".$tempparseLink['host'].")$/i", $parseLink['host'], $hosts)) {
				$flashinfo = $this->_video_getvideoinfo($link, $hosts[1]);
	        //}
        }
        
        if ($flashinfo['flashvar']) {
            $typedata['flashvar']  = $type_data;
            $typedata['flashimg']  = $flashinfo['img'];
            $typedata['host']      = $hosts[1];
            $typedata['source']    = empty($flashinfo['url'])?$type_data:$flashinfo['url'];
    ...
    private function _video_getvideoinfo($link, $host)
    {
        $return='';
        $parseLink = parse_url($link);
        $path = $parseLink['path'];
		...
		if (C('URL_ROUTER_ON')) {
			$i = strripos($path,"/");
	        $videoid = substr($path,$i+1);
			$video = D('Video','video')->where("id='".$videoid."'")->find();
			$file = D('OvFiles','video')->getByFcode($video['fcode']);
		}else{
	        $i = strripos($path,"/");
	        $filepath = substr($path,1,$i-1);
	        $outfilename = substr($path,$i+1);
	        $file = D('OvFiles','video')->getByPath_Name($filepath,$outfilename);
			$video = D('Video','video')->where("fcode='".$file['filecode']."' AND savepath='".$file['filedir']."'")->find();
		}

		@require_once(SITE_PATH.'/apps/video/Common/common.php');
        $return['flashvar'] = get_video_url($video['id'],1);
        $return['img']   = get_picture_url($video['id'],1);
        $return['title'] = $video['name'];
        $return['url'] = U('video/Index/video',array(id=>$video['id'],aid=>$video['categoryId'],uid=>$video['userId']));
        return $return;
    }
