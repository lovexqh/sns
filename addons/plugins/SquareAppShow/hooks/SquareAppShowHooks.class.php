...
class SquareAppShowHooks extends Hooks
{
	...
	public function public_head()
	{
		...
		.ico_blog, .ico_blog_on, .ico_photo, .ico_photo_on, .ico_video, .ico_video_on, .ico_library, .ico_library_on, .ico_group, .ico_group_on
		...
		.ico_video{background-position:-26px -303px}.ico_video_on{background-position:-26px -303px}
		.ico_library{background-position:-26px -321px}.ico_library_on{background-position:-26px -321px}
		</style>';
	}
	...
	private function _video()
	{
		...
		$map['privacy']	=	1; //所有人公开的视频

		//获取配置参数
		$config = getConfig();
		$data['videos'] = D('Video', 'video')->where($map)->order($order)->findPage(15);
		$data['video_preview'] = $config['video_preview'];
		$this->assign($data);
		$this->display('video');
	}
	...
	{
		require_once SITE_PATH . '/apps/library/Common/common.php';
		// 热门推荐文档
		$data['IsHotList'] = IsHotList();
		// 文档
		$order  = NULL;
		...
		$map['privacy']	=	1; //所有人公开的文档

		//获取配置参数
		$config = getConfig();
		$data['librarys'] = D('Library', 'library')->where($map)->order($order)->findPage(15);
		$data['library_preview'] = $config['library_preview'];
		$this->assign($data);
	...
	public function config()
	{
		...
		$apps_show_list = array('photo', 'blog', 'group','video','library');
