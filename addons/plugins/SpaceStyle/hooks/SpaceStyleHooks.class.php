...
class SpaceStyleHooks extends Hooks
{
    ...
    public function public_head($param)
    {
		...
		//检查UC数据是否完整
		if(!empty($_SESSION['ucInfo'])){
			$ucinfo = $_SESSION['ucInfo'];
			switch ($ucinfo['identitytype']) {
				case '2':	//老师
					echo '<link href="' . $this->htmlPath . '/html/teacher.css" rel="stylesheet" type="text/css" />';
					break;
				case '3':	//学生
					echo '<link href="' . $this->htmlPath . '/html/student.css" rel="stylesheet" type="text/css" />';
					break;
				case '4':	//家长
					echo '<link href="' . $this->htmlPath . '/html/garden.css" rel="stylesheet" type="text/css" />';
					break;
				default:
					;
				break;
    ...
    echo '<link href="' . $class_url . '" rel="stylesheet" type="text/css" id="change_skin" />';
		...
		//载入自定义背景
		$background['image'] && $background['image'] = "url('__ROOT__/{$background['image']}')";
		$background_CSS = array();
		foreach ($background as $key => $value) {
			$value && $background_CSS[$key] = "background-{$key}:{$value};";
		}
		if (!empty($background_CSS)) {
			echo '<style id="change_background">.containerOuter{' . implode('', $background_CSS) . '}</style>';
		}
	}
