...
/**
 * 新浪微广播 OAuth 认证类(OAuth2)
 *
 * 授权机制说明请大家参考微广播开放平台文档：{@link http://open.weibo.com/wiki/Oauth2}
 *
 * @package sae
 * @author Elmer Zhang
 * @version 1.0
...
class SaeTClientV2
{
	 ...
	 *
	 * @access public
	 * @param mixed $akey 微广播开放平台应用APP KEY
	 * @param mixed $skey 微广播开放平台应用APP SECRET
	...
	/**
	 * 获取当前登录用户及其所关注用户的最新微广播消息。
	 *
	 * 获取当前登录用户及其所关注用户的最新微广播消息。和用户登录 http://weibo.com 后在“我的首页”中看到的内容相同。同friends_timeline()
	 * <br />对应API：{@link http://open.weibo.com/wiki/2/statuses/home_timeline statuses/home_timeline}
	 *
	 * @access public
	 * @param int $page 指定返回结果的页码。根据当前登录用户所关注的用户数及这些被关注用户发表的微广播数，翻页功能最多能查看的总记录数会有所不同，通常最多能查看1000条左右。默认值1。可选。
	 * @param int $count 每次返回的记录数。缺省值50，最大值200。可选。
	 * @param int $since_id 若指定此参数，则只返回ID比since_id大的微广播消息（即比since_id发表时间晚的微广播消息）。可选。
	 * @param int $max_id 若指定此参数，则返回ID小于或等于max_id的微广播消息。可选。
	 * @param int $base_app 是否只获取当前应用的数据。0为否（所有数据），1为是（仅当前应用），默认为0。
	 ...
	 * 获取当前登录用户及其所关注用户的最新微广播消息。
	 *
	 * 获取当前登录用户及其所关注用户的最新微广播消息。和用户登录 http://weibo.com 后在“我的首页”中看到的内容相同。同home_timeline()
	 ...
	 * @param int $since_id 若指定此参数，则只返回ID比since_id大的微广播消息（即比since_id发表时间晚的微广播消息）。可选。
	 * @param int $max_id 若指定此参数，则返回ID小于或等于max_id的微广播消息。可选。
	 * @param int $base_app 是否基于当前应用来获取数据。1为限制本应用微广播，0为不做限制。默认为0。可选。
	 * @param int $feature 微广播类型，0全部，1原创，2图片，3视频，4音乐. 返回指定类型的微广播信息内容。转为为0。可选。
	 * @return array
	 */
	 ...
	 * 获取用户发布的微广播信息列表
	 *
	 * 返回用户的发布的最近n条信息，和用户微广播页面返回内容是一致的。此接口也可以请求其他用户的最新发表微广播。
	 ...
	 * @param mixed $uid 指定用户UID或微广播昵称
	 * @param int $since_id 若指定此参数，则只返回ID比since_id大的微广播消息（即比since_id发表时间晚的微广播消息）。可选。
	 * @param int $max_id 若指定此参数，则返回ID小于或等于max_id的提到当前登录用户微广播消息。可选。
	 * @param int $base_app 是否基于当前应用来获取数据。1为限制本应用微广播，0为不做限制。默认为0。
	 * @param int $feature 过滤类型ID，0：全部、1：原创、2：图片、3：视频、4：音乐，默认为0。
	 ...
	 *
	 * @access public
	 * @param string $screen_name 微广播昵称，主要是用来区分用户UID跟微广播昵称，当二者一样而产生歧义的时候，建议使用该参数
	 * @param int $page 页码
	 * @param int $count 每次返回的最大记录数，最多返回200条，默认50。
	 * @param int $since_id 若指定此参数，则只返回ID比since_id大的微广播消息（即比since_id发表时间晚的微广播消息）。可选。
	 * @param int $max_id 若指定此参数，则返回ID小于或等于max_id的提到当前登录用户微广播消息。可选。
	 ...
	 * 获取@当前用户的微广播列表
	 *
	 * 返回最新n条提到登录用户的微广播消息（即包含@username的微广播消息）
	 ...
	 * @param int $max_id 若指定此参数，则返回ID小于或等于max_id的提到当前登录用户微广播消息。可选。
	 * @param int $filter_by_author 作者筛选类型，0：全部、1：我关注的人、2：陌生人，默认为0。
	 * @param int $filter_by_source 来源筛选类型，0：全部、1：来自微广播、2：来自微群，默认为0。
	 * @param int $filter_by_type 原创筛选类型，0：全部微广播、1：原创的微广播，默认为0。
	 * @return array
	 */
	 ...
	 *
	 * @param string $ids 需要查询的微广播ID，用半角逗号分隔，最多不超过50个。
	 * @return array
	 */
	...
	/**
	 * 通过微广播（评论、私信）ID获取其MID
	 *
	 * 对应API：{@link http://open.weibo.com/wiki/2/statuses/querymid statuses/querymid}
	 *
	 * @param int|string $id  需要查询的微广播（评论、私信）ID，批量模式下，用半角逗号分隔，最多不超过20个。
	 * @param int $type  获取类型，1：微广播、2：评论、3：私信，默认为1。
	...
	/**
	 * 通过微广播（评论、私信）MID获取其ID
	 *
	 * 对应API：{@link http://open.weibo.com/wiki/2/statuses/queryid statuses/queryid}
	 *
	 * @param int|string $mid  需要查询的微广播（评论、私信）MID，批量模式下，用半角逗号分隔，最多不超过20个。
	 * @param int $type  获取类型，1：微广播、2：评论、3：私信，默认为1。
	 * @param int $is_batch 是否使用批量模式，0：否、1：是，默认为0。
	 ...
	 *
	 * @access public
	 * @param int $sid 转发的微广播ID
	 * @param string $text 添加的评论信息。可选。
	 * @param int $is_comment 是否在转发的同时发表评论，0：否、1：评论给当前微广播、2：评论给原微广播、3：都评论，默认为0。
	 * @return array
	 */
	 ...
	 *
	 * 发布一条微广播信息。
	 * <br />注意：lat和long参数需配合使用，用于标记发表微广播消息时所在的地理位置，只有用户设置中geo_enabled=true时候地理位置信息才有效。
	 * <br />注意：为防止重复提交，当用户发布的微广播消息与上次成功发布的微广播消息内容一样时，将返回400错误，给出错误提示：“40025:Error: repeated weibo text!“。
	 * <br />对应API：{@link http://open.weibo.com/wiki/2/statuses/update statuses/update}
	 *
	 * @access public
	 * @param string $status 要更新的微广播信息。信息内容不超过140个汉字, 为空返回400错误。
	 * @param float $lat 纬度，发表当前微广播所在的地理位置，有效范围 -90.0到+90.0, +表示北纬。可选。
	 * @param float $long 经度。有效范围-180.0到+180.0, +表示东经。可选。
	 * @param mixed $annotations 可选参数。元数据，主要是为了方便第三方应用记录一些适合于自己使用的信息。每条微广播可以包含一个或者多个元数据。请以json字串的形式提交，字串长度不超过512个字符，或者数组方式，要求json_encode后字串长度不超过512个字符。具体内容可以自定。例如：'[{"type2":123}, {"a":"b", "c":"d"}]'或array(array("type2"=>123), array("a"=>"b", "c"=>"d"))。
	 * @return array
	 */
	 ...
	 * 获取表情列表
	 *
	 * 返回新浪微广播官方所有表情、魔法表情的相关信息。包括短语、表情类型、表情分类，是否热门等。
	 ...
	 * @param int $count  单页返回的记录条数，默认为50。
	 * @param int $page 返回结果的页码，默认为1。
	 * @param int $filter_by_source 来源筛选类型，0：全部、1：来自微广播的评论、2：来自微群的评论，默认为0。
	 * @return array
	 */
	 ...
	 * @param int $id 需要评论的微广播ID。
	 * @param int $comment_ori 当评论转发微广播时，是否评论给原微广播，0：否、1：是，默认为0。
	 * @return array
	 */
	 ...
	 * 删除当前用户的微广播评论信息。
	 *
	 * 注意：只能删除自己发布的评论，发部微广播的用户不可以删除其他人的评论。
	 ...
	 *
	 * @access public
	 * @param mixed $target_name 目标用户的微广播昵称
	 * @param mixed $source_name 源用户的微广播昵称，可选，默认为当前的用户
	 * @return array
	 */
	 ...
	 * 支持设置的项：
	 *  - comment	int	是否可以评论我的微广播，0：所有人、1：关注的人，默认为0。
	 ...
	 * @param string $q 搜索的关键字，必须做URLencoding。必填
	 * @param int $count 返回的记录条数，默认为10。
	 * @param int $type 联想类型，0：我关注、1：关注我。必填
	 ...
	 *  - sort		int		排序方式，1：按更新时间、2：按被关注数，默认为1。
	 *  - count		int		单页返回的记录条数，默认为10。
	 *  - page		int		返回结果的页码，默认为1。
	 *  - base_app	int		是否只获取当前应用的数据。0为否（所有数据），1为是（仅当前应用），默认为0。
	 * 以上所有参数全部选填
