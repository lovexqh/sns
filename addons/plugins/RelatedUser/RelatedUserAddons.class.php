...
class RelatedUserAddons extends SimpleAddons
{
	...
	{
		return $this->apply('home_list_user_related','showRelatedUser');
	}
	...
	public function showRelatedUser($param)
	{
		...
		$data['limit']	 = isset($data['limit']) ? intval($data['limit']) : 5;
	...
	/**
	 * 可能认识的人 (可能认识的人 = 有相同tag的用户 || 所在城市相同的用户 || 好友的好友 || 关注我的 || 随机推荐)
	 *
	...
	public function getRelatedUser($uid, $max = 100, $do_shuffle = true)
	{
    	...
    	$follower_weight = isset($config['follower_weight']) ? intval($config['follower_weight']) : 1; // 关注我的
		...
		foreach ($related_uids as $k => $v) {
			...
			if (in_array($v, $tag_uids))
				...
				$related_uids[$k] = array('uid' => $v, 'reason' => '关注您的');
	 ...
	 * @param int   $limit        推荐的人数
	 * @return array 被推荐用户的uid数组
	 * @todo 还需要优化效率 (用户总数8000: 关注我和我关注各7000+时, 执行时间约500ms; 关注我和互相关注各2000+时, 执行时间约3ms)
	 */
