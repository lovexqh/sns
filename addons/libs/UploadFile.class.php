...
{//类定义开始
    ...
    private function save($file)
    {
        ...
        if($this->thumb) { // 生成图像缩略图
			...
			//缩图路径-文件名
			$bigpic		=	$file['savepath'].$file['savename'];
			$smallpic	=	$file['savepath'].'small_'.$file['savename'];
			$middlepic	=	$file['savepath'].'middle_'.$file['savename'];
			...
			$typedata['thumbmiddleurl'] = ($file['extension']=='gif')?$bigpic:$middlepic;
			$typedata['picurl']			= $bigpic;
            $typedata['attach_id']      = $file['id'];
			//为微广播缩略图-小图不加水印，大图、中图加水印
			//if($fileext!='gif'){
...
//             	}
            	//edit by  yangjs            	
                $this->error = '上传文件大小超过了php.ini配置upload_max_filesize的值'.$size.',请修改配置文件！';
            	break;
            case 2:
         		$size = ini_get("upload_max_filesize");
            	if( strpos($size,'M')!==false || strpos($size,'m')!==false ) {
