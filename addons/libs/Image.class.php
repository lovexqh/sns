...
//        @imagestring($im, 5, 5, 3, $randval, $stringColor);
     ...
     * @static
     +----------------------------------------------------------
     * @param string $picurl 图像位置
     * @param string $roll 旋转角度
     +----------------------------------------------------------
     */
    static function rotateChange($picurl,$roll=0){
    	$result = false;
		$imageInfo = getimagesize($picurl);
		//邵世强 add 14-08-12
		$j = 0;
		while(!$imageInfo && $j<100){
			$imageInfo = getimagesize($picurl);
			$j ++;
		}
		//邵世强 add end
	    if( $imageInfo!== false) {
	        ...
	        switch($imageType){
	        	case 'jpeg':
	        		...
	        		$result = @imagejpeg(imagerotate($img,$roll,imagecolorallocate($img,255,255,255)),$picurl);
	        		break;
	        	case 'gif':
	        		$img = imagecreatefromgif($picurl);
	        		$result = @imagegif(imagerotate($img,$roll,imagecolorallocate($img,255,255,255)),$picurl);
	        		break;
	        	case 'png':
	        		$img = imagecreatefrompng($picurl);
	        		$result = @imagepng(imagerotate($img,$roll,imagecolorallocate($img,255,255,255)),$picurl);
	        		break;
	        }
	    }
	    return $result;
