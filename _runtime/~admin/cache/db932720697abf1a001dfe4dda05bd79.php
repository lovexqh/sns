...
<script type="text/javascript" src="__PUBLIC__/js/jquery.datapicker.js"></script>
<script type="text/javascript" src="__PUBLIC__/js/jquery.datapicker.css"></script>
...
function uc_left_menu(uc_menu_data) {
	var leftmenu = $('#root_ucenter').find('ul');
	var id =$(leftmenu).find('a').attr('id');
	var menudata = id.split('_');
	$(leftmenu).find('li').each(function(index, element) {
        if(0 == index){
			leftmenu_innerHTML = '<li>'+$(this).html()+'</li>';
		}
    });	
	var html_str = '';
	var k=1;
	for(var i=2;i<uc_menu_data.length;i+=2) {
		html_str += '<li><a id="'+menudata[0]+'_'+(menudata[1]+k)+'" href="javascript:void(0)" onClick="switch_sub_menu(\''+(menudata[1]+k)+'\', \''+uc_menu_data[(i+1)]+'\');" class="submenuA" hidefocus="true" style="outline:none;">'+uc_menu_data[i]+'</a></li>';
		k++;
	}
	leftmenu_innerHTML += html_str;
	$(leftmenu).html(leftmenu_innerHTML);
}
...
<body scroll="yes" style="margin:0; padding:0;" onLoad="nof5()">
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td colspan="3">
		<div class="header"><!-- 头部 begin -->
		    ...
		    <div class="nav_sub">
		    	您好,<?php echo (getUserName($mid)); ?>&nbsp; | <a href="<?php echo U('home/Index/index');?>">返回前台</a> | <a href="javascript:void(0);" onClick="refresh();">刷新</a> | <a href="<?php echo U('home/Public/logoutAdmin');?>">退出</a><br/>
		    	<div id="TopTime"></div>
		    </div>
		    <div class="main_nav">
                ...
                <!------------------------徐程亮添加用于移动版本管理 start----------------------->
                <?php if($_GET[m]==1){ ?>
                <a id="channel_mobile" <?php if(($key)  ==  "index"): ?>class="on"<?php endif; ?> href="javascript:void(0)" onclick="switchChannel('mobile');" hidefocus="true" style="outline:none;">移动管理</a>
                <?php } ?>
                <!--------------------------徐程亮添加用于移动版本管理 end----------------------->
			</div>
		</div>
  ...
  <tr>
    ...
    <td>
   	  <iframe onload="nof5()" id="main" name="main" scrolling="yes" src="<?php echo ($home_url); ?>" width="100%" height="100%" frameborder="0" noresize> </iframe>
	</td>
  </tr>
</table>
</body>
