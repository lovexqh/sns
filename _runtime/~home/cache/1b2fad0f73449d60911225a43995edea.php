...
function changeverify(){
    ...
    var url = "<?php echo SITE_URL;?>/public/captcha.php";
...
function code(){
	var verify = $("#verify").val();
	if(verify != ''){
		$.post("<?php echo U('home/Public/code');?>",{verify:verify},function(data){
			if(data == 0){
				ui.error("验证码错误，请重新输入");
			}
		})
...
<div class="wrap_login">
  <div class="la fl">
    <div class="box_login">
      <form action="<?php echo U('home/Public/doAdminLogin');?>" name="reg" method="post"  class="nf lf" onsubmit="return checkVerify(this)">
			<dl>
				<?php if($uid){ ?>
					<dd><label>名　字：</label><?php echo (getUserName($uid)); ?></dd>
				<?php }else{ ?>
					<dd>
					   <label>帐　号：</label>
				...
				<dd>
				    <label>验证码：</label>
                    <div class="c1">
				 <input id="verify" onblur="code()"  class="txt" onfocus="this.className='txt2'" name="verify" value="" style="width:58px">
	...
	<!--
	<div class="footer_login">Copyright &copy; 2012-2014 <a href="http://www.ruijie-grid.com/" target="_blank" >山东锐杰网格信息技术有限公司</a> 版权所有</div>
	-->
</div>
