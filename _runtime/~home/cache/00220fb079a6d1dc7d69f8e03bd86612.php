				...
				'<div class="card_content">'+
					'<div class="card_name clearfix">'+
						'<dl class="name clearfix">'+
							...
							'<div>'+
								'<ul class="userdata">'+
								'<li><a href="{following_url}">我关注</a>({followed_count})</li>'+
								'<li><a href="{follower_url}">关注我</a>({followers_count})</li>'+
								'<li><a href="{space_url}">微广播</a>({weibo_count})</li>'+
								'</ul>'+
							'</div>'+
							'</dd>'+
						'</dl>'+
	...
	_this.parseShow = function(data,position){
		...
		if(isLogin) {
			...
			if( data['follow_state'] =='unfollow' ){
				$('#space_card_toolbar').html('<span><a href="javascript:;" class="btn_b" onclick="space_card_dofollow(\'dofollow\','+uid+',\''+data['uname']+'\')">加关注</a></span>');
			}else if(data['follow_state'] =='havefollow'){
				$('#space_card_toolbar').html('<span>已关注 | <a href="javascript:;" onclick="space_card_dofollow(\'unflollow\','+uid+',\''+data['uname']+'\')">取消</a></span> <a href="javascript:void(0)" onclick="weibo.quickpublish(\'@' + data['uname'] +'\');$(\'#space_card_content\').hide();">@TA</a><i class="vline">|</i><a href="javascript:void(0);" onclick="ui.sendmessage('+uid+');$(\'#space_card_content\').hide();">发私信</a>');
        ...
        if(typeof(top.OpenWindow)=='function'){
            $('#space_card_content').find('a').each(function(i, e) {
                if($(this).attr('uid')){
                ...
                if($(this).attr('rel')=='win'){
                    var title = $(this).attr('title') ? $(this).attr('title') : "个人主页";
                    var url = $(this).attr('href');
                    $(this).attr('href','javascript:void(0);');
                    $(this).bind('click',function(){
                        top.OpenWindow('url',url,title,'','titlebutton=close|max|min,width=1035,height=600');
                    });
                }
                if($(this).parent().parent().attr('class')=="userdata"){
                    var title = $(this).attr('title') ? $(this).attr('title') : $(this).text();
                    var href = $(this).attr('href');
                    $(this).attr('href','javascript:;');
                    $(this).bind('click',function(){
                        if($(this).text()=="微广播"){
                            top.OpenBrowser(href,data.uname+'的微广播', 'width=1024,height=600,titlebutton=close|max|min');
                        }else{
                ...
                if($(this).parent().html().indexOf('个人标签')>=0){
                    var title = $(this).attr('title') ? $(this).attr('title') : $(this).text();
                    var url = $(this).attr('href');
                    $(this).attr('href','javascript:;');
                    $(this).bind('click',function(){
