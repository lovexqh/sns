...
ui = window.ui ||{
	...
	loading:function(message){
		var message = (typeof(message) == "undefined")?"数据载入中...":message;
		var style = "html_load_box";
		var html   =   '<div class="" id="ui_messageBox" style="display:none;z-index:1000001">'
					   + '<div class="html_clew_box_close"><a href="javascript:void(0)" onclick="$(this).parents(\'.html_clew_box\').hide()" title="关闭">关闭</a></div>'
					   + '<div class="html_clew_box_con" id="ui_messageContent">&nbsp;</div></div>';
		var init      =  0;
	...
	/*
	  ...
	  班级成员私信
	*/
	sendclassmessage:function(touid){
		touid = touid || '';
		if(!touid)
		{ 
		  ui.error("该收件人尚未注册！");
		...
		ui.box.load(U('home/Message/classpost',['touid='+touid]), {title:'发私信'});
	},
	...
	getclass:function(prefix,init_style,init_p,init_c){
		var style = (init_style)?'class="'+init_style+'"':'';
		var html = '<select name="'+prefix+'_area" '+style+'><option>我的学校</option></select> '+
				'<select name="'+prefix+'_grade" '+style+'><option>我的级部</option></select> '+
				'<select name="'+prefix+'_class" '+style+'><option value=0>我的班级</option></select>';
		document.write(html);
		// _PUBLIC_+'/js/area.js'
		$.getJSON(U('home/Public/getArea'), function(json){
			json = json.provinces;
...
//视频缩略图不存在
function imgVideoErr() 
{ 
	var img=event.srcElement;
	img.src=_THEME_+'/member/images/novideo.gif'; 
	img.onerror=null;
}
//图片文件不存在
function imgPicErr(w) 
{ 
	var img=event.srcElement; 
	img.src=_THEME_+'/member/images/nopic.gif'; 
	img.width=w;
	img.onerror=null;
}
/**
 * 
 * @param appname 收藏的应用名
 * @param action 收藏应用的类名
 * @param method 收藏应用的方法名
 * @param fid 收藏id
 * @param appConfig = 应用机配置的基本数组
 */
function add_Favorite(appname,action,method,fid,appConfig){
	appConfig = arguments[4]?arguments[4]:'';
	if(appname != '' && fid != ''){
...
/**
 * @todo 更新收藏表里面的status状态列
 * @param appname 更新收藏应用名
 * @param action 更新收藏应用的类名
 * @param method 更新收藏应用的方法名
 * @param fid 更新收藏id
 * @param appConfig = 应用机配置的基本数组
 */
function up_Favorite(appname,action,method,fid,appConfig,status){
	appConfig = arguments[4]?arguments[4]:'';
	if(appname != '' && fid != ''){
		$.post(U('home/User/up_favorite'),
			   {appname:appname,action:action,method:method,fid:fid,appconfig:appConfig,status:status,time:new Date().getTime()},
			   function(data){
				  return data;
			   }
		);
...
/**
 * @todo 删除收藏表里面的status状态列
 * @param appname 删除收藏应用名
 * @param action 删除收藏应用的类名
 * @param method 删除收藏应用的方法名
 * @param fid 更新收藏id
 * @param appConfig = 应用机配置的基本数组
 */
function del_Favorite(appname,action,method,fid,appConfig){
	appConfig = arguments[4]?arguments[4]:'';
	if(appname != '' && fid != ''){
		$.post(U('home/User/del_favorite'),
			   {appname:appname,action:action,method:method,fid:fid,appconfig:appConfig,time:new Date().getTime()},
			   function(data){
				   return data;
			   }
