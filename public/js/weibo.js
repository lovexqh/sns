$.extend({
    weibo:function(setting){
        var defaultOpt = {
			...
			ClassloadNewDiv:'#ClasscountNew',
			ClassloadMoreDiv:'#ClassloadMoreDiv',
			retractDiv:'#retract',
            publishForm:{
                type:"input[name=publish_type]",
				//simple:"input[name=simple]",
        ...
        $.extend(this,{
				...
				ClasscountNew:function(){
                    if(_MID_ <= 0){
                        return ;
                    }
                    if(!opt.show_feed && opt.lastId>0){
                        setInterval(function(){
                            operateFactory.create('ClasscountNew',function(txt){
                                if(txt.indexOf('<TSAJAX>')==0) {
                                    if(txt.indexOf('<HASNEW>')!=-1) {
                                        $('#ClasscountNew').html(txt);
                                        events.ClassloadNew();
                                    }
                                }else{
                                    //location.reload();
        ...
        function OperateFactory(nowType){
            ...
            var weibo={
				...
				ClasscountNew:{
                   url:U('weibo/index/Classcountnew'),
                   param:{lastId:'lastId',showfeed:'show_feed',type:'weiboType',follow_gid:'follow_gid',simple:'4',classid:'classid'}
                },
                loadNew:{
                    url:U('weibo/index/loadnew'),
                    param:{since:'lastId',showfeed:'show_feed',type:'weiboType',follow_gid:'follow_gid',simple:'3'}
                },
				ClassloadNew:{
                    url:U('weibo/index/Classloadnew'),
                    param:{since:'lastId',showfeed:'show_feed',type:'weiboType',follow_gid:'follow_gid',simple:'4',classid:'classid'}
                },
                deleteWeibo:{
                ...
                loadMore:{
                    ...
                    param:{since:'sinceId',showfeed:'show_feed',type:'weiboType',follow_gid:'follow_gid',simple:'3'}
                },
				ClassloadMore:{
                    url:U('weibo/index/Classloadmore'),
                    param:{since:'sinceId',showfeed:'show_feed',type:'weiboType',follow_gid:'follow_gid',simple:'4',classid:'classid'}
                }
            }
            ...
            var all={
                ...
                loadNew:{
                    url:U('weibo/index/loadnew'),
                    param:{hasUid:false,since:'lastId',showfeed:'show_feed',type:'weiboType',follow_gid:'follow_gid','simple':3}
                },
                comment:{
                ...
                loadMore:{
                    url:U('weibo/index/loadmore'),
                    param:{hasUid:false,since:'sinceId',showfeed:'show_feed',type:'weiboType',follow_gid:'follow_gid','simple':3}
                }
            }
        ...
        after_publish = function(txt){
				...
				//$('#feed_list').prepend((txt.substring(23,txt.indexOf('</ul>')))).slideDown('slow');
        ...
        var events = {
                ...
                lodeMore:function(){
                     $(opt.loadMoreDiv).click(function() {
                        ...
                        operateFactory.create("loadMore",function(txt){
                            ...
                            //判断没有更多数据时.不显示更多按钮
                            if(txt.indexOf('<HASNEW>')==-1){
                                $(self).html('<span class="morefoot">没有更多数据了</span>');
                            }else{
                                if(loadMoreCount<1){
				...
				ClasslodeMore:function(){
                     $(opt.ClassloadMoreDiv).click(function() {
                        $(this).html('加载中...');
                        var self = this;
                        loadMoreCount = typeof(loadMoreCount) == 'undefined' ? 0 : loadMoreCount;
                        operateFactory.create("ClassloadMore",function(txt){
                            clearInterval(isLoading);
                            isLoading = false;
                            loadMoreCount++ ;
                            if(parseInt(opt.sinceId) !== 0) {
				...
				//收起操作
				retract:function(){
                     $(opt.retractDiv).click(function() {
						 if(typeof(opt._sinceId) != 'undefined'){
							var self = $(this).parent();
							var length = $(self).parent().find('ul>li').length;
							 if(length>5){
								 $(self).parent().find('ul>li').each(function(i, e) {
									if(i>=5){
										$(this).slideUp(500,function(){$(this).remove();});
									}
								});
								opt.sinceId = opt._sinceId;
							 }
							 $(self).find(opt.loadMoreDiv).html('<span class="ico_morefoot"></span>更多');
						 }else{
							ui.success('暂无展开更多记录，无法进行收起操作！'); 
						 }
						 $(opt.retractDiv).hide();
					 });
                },
