...
session_start();
$name= 'verify';
$font 	= '../addons/libs/WaterMark/AbcManuscript.ttf';
...
if(file_exists($font) && isset($_GET['type']) && $_GET['type']=='chinese'){
	...
	Image::buildImageVerify(4, 1, 'png', 60, 25, $name);
}
...
{//类定义开始
    ...
    static function buildImageVerify($length=4,$mode=1,$type='png',$width=48,$height=22,$verifyName='verify')
    {
        ...
        for($i=0;$i<3;$i++){
			...
			//imagearc($im,mt_rand(-10,$width),mt_rand(-10,$height),mt_rand(30,300),mt_rand(20,200),55,44,$fontcolor);//化成弧线
			imagearc($im,mt_rand(-10,$width),mt_rand(-10,$height),mt_rand(30,300),mt_rand(20,200),40,30,$fontcolor);
		}
		for($i=0;$i<30;$i++){
			$fontcolor=imagecolorallocate($im,mt_rand(0,255),mt_rand(0,255),mt_rand(0,255));
			imagesetpixel($im,mt_rand(0,$width),mt_rand(0,$height),$pointColor);//绘点
		}
		for($i=0;$i<$length;$i++) {
			//imagestring($im,5,$i*10+5,mt_rand(1,8),$randval{$i}, $stringColor);//水平地画一行字符串
			imagestring($im,5,$i*10+10,mt_rand(1,8),$randval{$i}, $stringColor);
		}
//        @imagestring($im, 5, 5, 3, $randval, $stringColor);
	...
	static function GBVerify($length=4,$type='png',$width=120,$height=30,$fontface='simhei.ttf',$verifyName='verify') {
		...
		for($i=0;$i<255;$i++){
			$fontcolor=imagecolorallocate($im,mt_rand(0,255),mt_rand(0,255),mt_rand(0,255));//字体颜色(随机)
