...
class control extends adminbase {
	...
	function control() {
		...
		$this->load('user');
		$this->check_priv();
		if(!$this->user['isfounder'] && !$this->user['allowadminesnadmin']) {
	...
	function onls(){
		$this->view->display('admin_esnadmin');
	}
...
/*-------------------------------------------------------------------------------------*/
	...
	function ongetEsnAdmin(){
		$keyvalue = $_POST['key'];
		$pageSize = $_POST['pageSize'];
		$pageIndex = $_POST['pageIndex'];
		$jsonstr = json_encode( $_ENV['esnadmin']->getEsnAdmin($keyvalue,$pageIndex,$pageSize));
		print_r($jsonstr);
	}
	...
	function onTeacherTree(){
		...
		echo $_ENV['esnadmin']->getTeacherDeptTree($logschoolid);
	}
	...
	function ongetMemberList(){
		...
		$pageSize = $_POST['pageSize'];
		$pageIndex = $_POST['pageIndex'];
		$memberList =  $_ENV['esnadmin']->getMemberList($logschoolid, $schoolid, $updepid, $deptid,$keyvalue,$pageIndex,$pageSize);
	...
	function onaddadmin(){
		$uid = $_POST['uid'];
		$uid = explode(',', $uid);
		$_ENV['esnadmin']->addadmin($uid);
	}
	
	function ondelAdmin(){
		$id = $_POST['id'];
		$_ENV['esnadmin']->delAdminById($id);
	}
}
