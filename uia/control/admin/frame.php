...
class control extends adminbase {
	...
	function onpwdChg(){
		...
		$this->view->display("admin_password_change");
	}
	...
	function onchgPwd(){
		...
		$md5oldpwd = md5($rows[0]['oldpwd']);
		$md5newpwd = md5($rows[0]['newpwd']);
		$md5newpwd2 = md5($rows[0]['newpwd2']);
		$username = $_GET['username'];
		$result = $this->chkPassword($username, $md5oldpwd);
		if($result == 1){	//原密码正确，执行更新操作
			$this->upPassword($username, $md5newpwd);
	...
	function upPassword($username, $md5newpwd){
		$upsql = "UPDATE ".UC_DBTABLEPRE."sysadmin SET password='$md5newpwd' WHERE username='$username'";
		$this->db->query($upsql);
	}
	
	function chkPassword($username, $password){
		$sql = "SELECT password FROM ".UC_DBTABLEPRE."sysadmin WHERE username='$username'";
		$admin = $this->db->fetch_first($sql);
