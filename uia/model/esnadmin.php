...
class esnadminmodel {
	...
	function __construct(&$base) {
		$this->esnadminmodel($base);
	}

	function esnadminmodel(&$base) {
		$this->base = $base;
		$this->db = $base->db;
	...
	function get_newadmin_list_grid($keyvalue="", $page=0, $pagesize=50)
	{
		$sql = "";
		if($keyvalue != ''){
			$sql .= " AND (username LIKE '%$keyvalue%')";
		}
		$index = $page * $pagesize;
		$sum = $this->db->fetch_first( "SELECT COUNT(*) as num FROM ".UC_DBTABLEPRE."sysadmin a,".UC_DBTABLEPRE."schoolinfo s WHERE a.schoolid=s.id ".$sql);
		$data['total'] = $sum['num'];
		$sql = "SELECT a.*,s.xxmc FROM ".UC_DBTABLEPRE."sysadmin a,".UC_DBTABLEPRE."schoolinfo s WHERE a.schoolid=s.id".$sql." order by a.id DESC LIMIT $index,$pagesize";

		$data['data'] =  $this->db->fetch_all($sql);
		for($i=0; $i<$data['total']; $i++){
			if($data['data'][$i][adminType]==0){
				$data['data'][$i][adminType] = "超级管理员";
			}else if($data['data'][$i][adminType]==1){
				$data['data'][$i][adminType] = "学校管理员";
			}
		}
		return $data;
	}
...
// 	}
	...
	function getEsnAdmin($keyvalue,$pageIndex,$pagesize){
		$sql = "";
		if($keyvalue != ''){
			$sql .= " AND username LIKE '%$keyvalue%'";
		}
		$index = $pageIndex * $pagesize;
		$sum = $this->db->fetch_first( "SELECT COUNT(*) as num FROM ".UC_DBTABLEPRE."sysadmin s left join ".UC_DBTABLEPRE."members m on s.username=m.email WHERE s.adminType='1' ".$sql);
		$data['total'] = $sum['num'];
		$sql = "SELECT s.id,s.username,s.schoolid,sch.xxmc,s.`name`,s.contact,m.email FROM ".UC_DBTABLEPRE."sysadmin s left join ".UC_DBTABLEPRE."members m on s.username=m.email left join ".UC_DBTABLEPRE."schoolinfo sch on s.schoolid=sch.id WHERE s.adminType='1' ".$sql." ORDER BY s.id DESC LIMIT $index,$pagesize";
		$data['data'] =  $this->db->fetch_all($sql);
		return $data;
	}
...
// 			$this->db->query($sql2);
	...
	function delAdminById($id){
		$delstr = "DELETE FROM ".UC_DBTABLEPRE."sysadmin WHERE id in ($id)";
		$delstr2 = "DELETE FROM ".UC_DBTABLEPRE."uia_r_user_role WHERE uid in ($id)";
		$this->db->query($delstr);
