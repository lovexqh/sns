...
class base {
	...
	function getLoginUserSchoolId(){
		...
		$member=$this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE username='$username'");
		if(!$member){
			$sysadmin = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."sysadmin WHERE username='$username'");
			if(!$sysadmin){
				return -1;
			}else{
				$schoolId = $sysadmin['schoolid'];
			}
		}else{
			$identityType=$member['identityType'];	//获得用户身份信息
			$identityid=$member['identityID'];		//获得用户信息标识号
			$baseInfo=null;
			
			$tableName="";
			if($identityType==1){//学生身份,则查询学生基础信息表
				$tableName=UC_DBTABLEPRE."studentinfo";
			}elseif($identityType==2){//老师身份,则查询老师基础信息表
				$tableName=UC_DBTABLEPRE."teacherinfo";
			}else{
				return -1;
			}
			$baseInfo=$this->db->fetch_first("SELECT * FROM ".$tableName." WHERE identityid='$identityid'");
...
// 		if($uid==-1){//如果是超级管理员登录,则返回-1
// 			return -1;
// 		}
// 		$member=$this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE uid='$uid'");
// 		if(!$member){
// 			return -1;
// 		}
...
// 		}
	 ...
	 * 如果是超级管理员,则返回所有学校信息
	 */
	function getLoginUserSchool(){
		$schoolId=$this->getLoginUserSchoolId();
		$condition="";
		if($schoolId!=-1){//非超级管理员登录,则加上学校id作为筛选条件
			$condition=" WHERE id='$schoolId'";
		}
		$sql="SELECT id as schoolid,xxmc as title FROM ".UC_DBTABLEPRE."schoolinfo ".$condition;
		$schools = $this->db->fetch_all($sql);
		return $schools;
	}

	function getSchoolName($schoolid){
		$sql="SELECT id as schoolid,xxmc FROM ".UC_DBTABLEPRE."schoolinfo WHERE id='$schoolid'";
		$school = $this->db->fetch_first($sql);
		return $school['xxmc'];
	}
	...
	function getLoginUserID(){
		$uid=$this->user['uid'];
		if(!$uid){
			return -1;//登录用户如果没有uid，则说明是超级管理员，返回uid=-1
		}
		return $uid;
	}
