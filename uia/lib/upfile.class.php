...
class UploadFile {
	...
	//默认允许用户上传的文件类型
	protected $allow_type = array('gif', 'jpg', 'png', 'zip', 'rar', 'txt', 'doc', 'pdf','docx','xls','xlsx');
	...
	function upload() {
		...
		for ($i = 0; $i < count($this->user_post_file['name']); $i++) {
			...
			if ($this->user_post_file['error'][$i] == 0) {
				...
				//移动后的文件名
				//$saveas = $basename.date('ymdHis').".".$type;
				$saveas = date('ymd').time().".".$type;
		...
		//return $this->final_file_path; //返回上传的文件路径
	}
