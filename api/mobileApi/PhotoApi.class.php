...
class PhotoApi extends Api{
    ...
    public function allPhotos(){
        ...
        $limit = getLimit();
        $a = M('photo')->where($map)->order($order)->field("id,userId,name,cTime,mTime,IFNUll(info,' ') info,commentCount,readCount,savepath")->limit($limit)->findAll();
        foreach($a as $k=>$v){
            if(!$v['info']){
                $a[$k] = '';
            }
            if(file_exists(SITE_DATA_PATH.'/uploads/'.$v['savepath'])){
                list($a[$k]['width'],$a[$k]['height']) = getimagesize(SITE_DATA_PATH.'/uploads/'.$v['savepath']);
            }
        }
        return $a;
    }
    ...
    function readCount(){
        if($id = (int)$_REQUEST['id']){
            M('photo_album')->setInc('readCount','id='.$id);
            return 1;
        }else{
            return 0;
        }
...
//        var_dump($map);
        ...
        unset($photos['html']);
        if($photos['data']){
            foreach($photos['data'] as $k=>$v){
                list($photos['data'][$k]['width'],$photos['data'][$k]['height']) = getimagesize(SITE_DATA_PATH.'/uploads/'.$v['savepath']);
            }
            return $photos;
        }else{
            return array();
    ...
    function deletePhoto(){
        ...
        if(D('Album','photo')->deletePhoto($pid,$this->mid)){
        	//社交圈  删除图片  photo_link   ssq  2014-02-29  add
        	D('PhotoLink','photo')->delPhoto(array('photoid'=>$pid));
            return 1;
        }else{
            return 0;
        }
    ...
    function uploadPic(){
        if(!$_POST['type'])exit;
        $_FILES['pic']['type']="image/".$_POST['type'];
        $_FILES['pic']['name']=$_FILES['pic']['name'].'.'.$_POST['type'];
        $c = getConfig();
        $_POST['config'] = $c;
        if(!$_REQUEST['albumId']){
...
//        $_POST['name'] = rand(1,100);
        if(!t($_POST['name']))exit;
        //BaseAction
        return A('Manage','photo')->do_create_album();
    }

    function deleteAlbum(){
        $id = (int)$_REQUEST['aid']?(int)$_REQUEST['aid']:exit;
        $uid = $this->mid;
        $result	= D('Album','photo')->deleteAlbum($id,$uid);
        if($result){
            return 1;
        }else{
            return 0;
