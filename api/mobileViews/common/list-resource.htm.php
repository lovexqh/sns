    ...
    <div id="scroller">
        ...
        <ul id="thelist">
            ...
            if($data){
                ...
                foreach ($data as $v) {
                    ?>
                    <li data-url="<?php echo $v['saveaddress'].$v['savepath'].$v['savename'];?>">
                        <section>
                            <div class="left" data-uid="<?php echo $v['author_id'];?>" data-name="<?php echo $v['author_name']?>">
                                <img src="<?php echo $v['img'];?>" data-path="<?php echo $v['img'];?>" class="video-p" data-uid="<?php echo $v['author_id'];?>" data-name="<?php echo $v['author_name']?>">
                            </div>

                            <div class="right">
                                <h3><?php echo $v['author_name'],'：',$v['title'];?></h3>
                                <div class="content">简介：<?php echo $v['info']?$v['info']:'还没有简介哦~';?></div>
                                <div class="down"><span class="l"><?php echo $v['cTime']?></span><span class="r"><?php echo $v['readCount'];?>看过</div>
                            </div>
                            <div class="go">
...
//        });
        //详情
        $("#wrapper").hammer({}).on("tap","li",function(){window.location="objc://resource^"+$(this).data('url');});
        //请求开始
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                for(var i in data){
                    ...
                    html = '<li data-url='+data[i]['saveaddress']+data[i]['savepath']+data[i]['savename']+'>';
                    html += '<section>';
                    html += '<div class="left" data-uid='+data[i]['author_id']+'>';
                    html += '<img class="video-p" data-uid='+data[i]['author_id']+' src='+data[i]['img']+'>';
                    html += '</div>';
                    html += '<div class="right">';
                    html += '<h3>'+data[i]['author_name']+'：'+data[i]['title']+'</h3>';
                    html += '<div class="content">'+info+'</div>';
                    html += '<div class="down"><span class="l">'+data[i]['cTime']+'</span><span class="r">'+data[i]['readCount']+'看过  </div>';
                    html += '</div>';
                    html += '<div class="go">';
