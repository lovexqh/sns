...
class ForeAdminAction extends ForeAdministratorAction {
	...
	public function applist() {
		...
		$admin = $_GET ['admin'];
		if($admin == 'audit')
			$where[$config['statusFieldName']] = 0;
		else if($admin == 'audited')
			$where[$config['statusFieldName']] = array('NEQ',0);
		

		$list = M ( $config['table'] )->where ( $where )->findPage (10);
		...
		switch ($actions) {
			...
			case 'unpass':
				...
				if($msg){
					...
					$arr['to'] = $info['uid'];
					$arr['title'] = '您发布的投票被驳回';
					$arr['content'] = '您发布的投票“'.U('vote/Index/index',array("id"=>$info['id'])).'”,理由：'.$msg ;
	...
	public function applist2() {
		$config = $this->config;
	
		$admin = $_GET ['admin'];
		if($admin == 'audit') $where[$config['statusFieldName']] = 0;
	...
	public function applist2_admin() {
		$config = $this->config;
