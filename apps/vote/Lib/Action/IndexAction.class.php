...
class IndexAction extends Action {
     ...
     */
        ...
        function add() {
            ...
            //$data['status'] = service('ForeAdmin')->isAuditApp() ? '0' : '1';
            ...
            if($result){
                ...
                X('Credit')->setUserCredit($this->mid,'add_vote');
                $this->assign('jumpUrl', U('vote/Index/pollDetail',array('id'=>$result,'iframe'=>'yes')));
                $this->ajaxData['url'] = U('vote/Index/pollDetail',array('id'=>$result,'iframe'=>'yes'));
        ...
        function index() {
        	global $ts;
				...
				if($_GET['searchtxt'] != ''){
					$map['title'] = array('like','%'. $_GET['searchtxt'] .'%');
				}
				//$map['status'] = 1;
        ...
        function my() {
			global $ts;
                ...
                //我参与的
                    ...
                    $this->setTitle("我参与的投票");
                }else {
                	$votes = $voteDao->where( $map  )->order( 'id DESC' )->findPage(getConfig('limitpage'));
                	$this->setTitle("我的投票管理");
                }
				//选项
                $optDao = D("VoteOpt");
                foreach($votes['data'] as $k=>$v) {
      ...
      */
        ...
        function deleteVote() {
                ...
                if( D( 'Vote' )->doDeleteVote( $id )) {
					X('Credit')->setUserCredit($this->mid,'create_vote','-');
                    echo 1;
                }else {
                    echo -1;
                }
        ...
        public function favorite(){
        	$data =model('Favorite')->list_favorite_all('vote',10);
