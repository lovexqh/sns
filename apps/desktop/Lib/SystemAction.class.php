...
class SystemAction extends BaseAction {
	...
	private function initApps() {
		...
		// 获取个人桌面图标列表
		$myicos = M ( 'dsk_icos' )->field ( "icoid,type,oid" )->where ( "uid=-1 or uid=" . $this->mid )->findAll ();
		
		$update = 0;
		foreach ( $myicos as $icos ) {
			// 如果现在桌面及文件夹中不存在该图标则添加
			if (! in_array ( $icos ['icoid'], $icos_0 ) && ! in_array ( $icos ['icoid'], $icos_f )) {
				$screenlist [1] ['icos'] [] = $icos ['icoid'];
				$update = 1;
			}
		}
		...
		if (count ( $screenlist_array ) != count ( $_myicos )) {
			...
			// dump($_myicos);
			$screenlist [1] ['icos'] = array_merge ( $screenlist [1] ['icos'], $_myicos );
			$update = 1;
		}
		// dump($screenlist);
		if ($update) { // 重新更新桌面图标
		...
		if ($this->mid) {
			...
			if ($config_u) {
    			...
    			*/
				...
				/**
				 * 1、这里面先判断一个插件表里面有没有 插件的信息
				 * 2、判断一个userconfig表里面的插件信息有没有 插件信息
				 * 3、如果有就不做操作，如果没有就把插件信息插入到初始屏幕中初始位置 
				 * 
				 */
				//1、得到插件表里面的当前用户的插件信息
				$widgetTable = M('dsk_widget')->where("uid = '$this->mid' AND notdelete > 0")->select();
				$widgetar = array();
				foreach ($widgetTable as $widgett=>$widget){
					array_push($widgetar, $wi['gid']);
				}
				 ...
				 */
				foreach ( $screenlist_u as $key => $value ) {
					$screenlist_u[$key]['widgetid']= array();
					foreach ($value['widgets'] as $wi=>$wivalue){
						array_push($screenlist_u[$key]['widgetid'], $wi);
					}
					
					/*
					 * desktop_move_userdata 函数的作用：是为了，把多于规定桌面数量的桌面里面的图标，插件，wins合并
					 */
				...
				foreach ($screenlist_u as $key=>$value){
					if($value['widgetid']){
						foreach ($value['widgetid'] as $wi=>$wivalue){
							if(!in_array($wivalue, $widgetar)){
								unset($screenlist_u[$key]['widgets'][$wivalue]);								
							}
						}
					}
				}
				...
				/*
				 * $needsave_screenlist = 1的时候，有一种情况 后台规定的屏幕数量发生的变化时，这里面要把新的系统配置更新到用户配置里面
				 */
				...
				/*
				 * @author Rickeryu @description 这个else里面的主要功能，就是当用户登录的时候，但是没有来过desktop应用的时候， 在ts_dsk_userconfig这个表里面没有基本的信息，就是要把初始化的数组信息生成字符串的形式存入到ts_dsk_userconfig表
				 */
    			/*
    			 * 初始化屏幕操作
				 ...
				 */
				$result = M ( 'dsk_widget' )->where ( "" )->findAll ();
				/*
