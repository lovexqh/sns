...
class UserModel extends Model {
	 ...
	 * @param string 	   $order		           结果排序
	 * @param int 		   $limit		 	限制条数
	 * @return array	        查询后的结果集
	 * @author 小波
	 +----------------------------------------------------------
	 * 创建时间：2013-3-5 15:20:33
	 +----------------------------------------------------------
	 */
     ...
     * 删除用户
     +----------------------------------------------------------
     * @param array|string $uids	用户ID
     * @return boolean	操作后的成功或失败
     * @author 小波
     +----------------------------------------------------------
     * 创建时间：2013-3-5 15:21:39
     +----------------------------------------------------------
     */
	 ...
	 * 更新基本信息
	 +----------------------------------------------------------
	 * @return	array	返回更新后的状态及信息
	 * @author	小波
	 +----------------------------------------------------------
	 * 创建时间：2013-3-5 15:22:57
	 +----------------------------------------------------------
	 */
	 ...
	 * @param bool	 $space	  是否为空间调用
	 * @return array 获取后的结果集
	 * @author 小波
	 +----------------------------------------------------------
	 * 创建时间：2013-3-5 15:23:36
	 +----------------------------------------------------------
	 */
	protected function data_field($module = '',$space=false){
		//读取用户的分组信息
		$map['uid'] = $this->uid;
		$group_link = M('user_group_link')->where($map)->findAll();
		$group_ids[] = '0';
		foreach ($group_link as $group){
			$group_ids[] = $group['user_group_id'];
		}
		unset($map);
		$map['status'] = 1;
		$map['utypeid'] = array('in',$group_ids);
		if($space){
			$map['showspace'] = 1;
			$list = $this->table(C('DB_PREFIX').'user_set')->where($map)->findAll();
		}else{
			$list = $this->table(C('DB_PREFIX').'user_set')->where($map)->findAll();
		}
	 ...
	 * @return array	   获取后的用户结果集
	 * @author 小波
	 +----------------------------------------------------------
	 * 创建时间：2013-3-5 15:24:51
	 +----------------------------------------------------------
	 */
	public function getUserByIdentifier($identifier, $identifier_type = 'uid')
	{
		if ($identifier_type == 'uid' && !is_numeric($identifier))
			return false;
		else if (!in_array($identifier_type, array('uid','uname','email','domain','sfzh','mobile')))
			return false;
     ...
     * 获取用户基本信息缓存
     +----------------------------------------------------------
     * @param int $uid 用户UID，string
	 * @param $type 查询类型(uid,uname,email,domain)
	 * @return array 用户基本信息
     * @author 小波
     +----------------------------------------------------------
     * 创建时间：2013-3-5 15:28:23
     +----------------------------------------------------------
     */
	public function getUserInfoCache($uid, $type = 'uid') {
		...
		if(!in_array($type, array('uid','uname','email','domain','sfzh','mobile'))) {
			return false;
		}
		
		/**
		 * Rickeryu 添加，用来得到基础数据里面的身份证号 - start
		 */
		if($type == 'sfzh'){
			$usersfzh = get_all_by_sfzh($uid);
			$uid = M('ucenter_user_link')->where('uc_uid = '.$usersfzh['uid'])->getField('uid');
			$type = 'uid';
		}
		/**
		 * Rickeryu 添加，用来得到基础数据里面的身份证号 - end
		 */
		//xiawei 添加 手机号登录 - start
		if($type == 'mobile'){
			$uid = $this->where('mobile='.$uid)->getField('uid');
			if(!$uid)
				return null;
		}
		//xiawei 添加 手机号登录 - end
		...
		if(empty($userInfo)) {
			...
			//用户身份
			$role = M('user_group_link')->where('uid='.$uid)->field('user_group_id')->findAll();
        	arrayFormat($role, $result);
			$userInfo['role'] = implode(',', $result);
...
// 		exit();
	...
	* @author Ricker lhyfe@sohu.com
	 */
	function getUcMember($user_id){
		$Center_user = D('ucenter_user_link');
		$user = $Center_user->where("uid = $user_id")->find();
		//echo $Center_user->getLastSql();
		return $user['uc_uid'];
	}
	
	function getCourseByUid($user_id){
		$memberid = $this->getUcMember($user_id);
		include_once(SITE_PATH.'/api/uc_client/uc_client.php');
		return uc_get_course_by_stid($memberid);
	}
}