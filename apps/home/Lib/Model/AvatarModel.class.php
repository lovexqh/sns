...
class AvatarModel extends Model{
	 ...
	 * 获取头像保存的地址
	 +----------------------------------------------------------
	 * @return	string 返回当前用户的头像地址
	 * @author	小波
	 +----------------------------------------------------------
	 * 创建时间：2013-3-5 15:40:54
	 +----------------------------------------------------------
	 */
     ...
     * 将远程图片转换成本地头像
     +----------------------------------------------------------
     * @param	int		$uid		用户ID
     * @param	string	$faceurl	远程地址
     * @author	小波
     +----------------------------------------------------------
     * 创建时间：2013-3-5 15:41:33
     +----------------------------------------------------------
     */
     ...
     * 上传头像
     +----------------------------------------------------------
     * @return	JSON	返回json格式的状态集合
     * @author	小波
     +----------------------------------------------------------
     * 创建时间：2013-3-5 15:42:41
     +----------------------------------------------------------
     */
    function upload(){
        @header("Expires: 0");
        ...
        if(empty($_FILES['Filedata'])) {
	        ...
	        if(@copy($_FILES['Filedata']['tmp_name'], $pic_path) || @move_uploaded_file($_FILES['Filedata']['tmp_name'], $pic_path)) 
	        {
	        	...
	        	include( SITE_PATH.'/addons/libs/Image.class.php' );
	        	$results = Image::thumb( $pic_path, $pic_path , '' , 300 , 300 );
				$imginfo = @getimagesize($pic_path);
				$i = 0;
				while(!$imginfo && $i <100){
					$imginfo = @getimagesize($pic_path);
					$i ++;
	        	...
	        	list($sr_w, $sr_h, $sr_type, $sr_attr) = $imginfo;
     ...
     * 获取摄像头头像（目前好像没有启用）
     +----------------------------------------------------------
     * @return	echo	直接显示获取到的所有数据
     * @author	小波
     +----------------------------------------------------------
     * 创建时间：2013-3-5 15:44:17
     +----------------------------------------------------------
     */
    function getcamera(){
        @header("Expires: 0");
