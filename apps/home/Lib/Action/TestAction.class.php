...
class TestAction extends Action{
	...
	public function testService(){
		$result = get_organization_by_schoolid();
		dump($result);
	}
	public function testCall(){
		$result = procExecute("call vcCategory(2268);");
		print_r($result);
	}
	...
	public function testU(){
		$result = M('blog')->where('ID=4')->findAll();
		dump($result);
	}
	...
	/**
	 * Enter description here ...<功能的描述>
	 * @author 小波
	 * @param <按照参数定义顺序(没有参数的方法就不用该选项)> 
	 * @return <返回类型(void的方法就不用该选项)>
	 * @throws Exception	<异常> 
	 * @version <当前版本>
	 * create Date: 2013-2-28
	 */
	public function test_uc_deptinfo(){
		$ucInfo = $_SESSION['ucInfo'];
		$result = uc_dept_get_uid(89);
		dump($result);
	}
