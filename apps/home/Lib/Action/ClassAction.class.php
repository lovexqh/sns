...
class ClassAction extends Action {
    ...
    function _initialize() {
		...
		$class_user = $this->get_class_list();

        $this->assign('studetns',array_slice($class_user,0,6));
        $this->assign('studetncount',count($class_user));
        $_SESSION['class']['list'] = $class_user;
	...
	//下一组 我的同学们
	public function replaceRelatedStudent()
	{
		if(!isset($_SESSION['class']['page'])){
			$_SESSION['class']['page']=2;
		}

		$page = intval($_SESSION['class']['page']);
		$totalnum = count($_SESSION['class']['list']);
		$totalpage = ceil($totalnum / 6);
		$page =  max(1, min($totalpage,intval($page)));

		if($page>=$totalpage){
			$_SESSION['class']['page']=1;
		}else{
			$_SESSION['class']['page']=$page+1;
		}

		$class_user = array_slice($_SESSION['class']['list'],(($page-1)*6),6);
        //获取我的同学全部数据
        $mails = array();
		$html  = '';
		foreach ($class_user as $result) {
	        $html .= '<li>';
	        $html .= 	'<a title="" href="'.U("home/Space/index",array("uid"=>$result['uid'])).'" rel="face" uid="'.$result['uid'].'" class="userface" target="_self">';
	        $html .= 		'<img src="'.getUserFace($result['uid'],'m').'">';
	        $html .=	'</a>';
	        $html .=	'<a class="fn" target="_self" rel="face" uid="'.$result['uid'].'" href="'.U("home/Space/index",array("uid"=>$result['uid'])).'">'.getUserName($result['uid']).'</a>';
	        $html .= '</li>';
		}
        echo $html;
    ...
    function get_class_list($field="*"){
		...
		$map['email'] = array('in',$mails);
		return D("User")->field($field)->where($map)->findAll();
    }
    ...
    function index() {
		...
		$data['hotUser'] = S('S_class_hotUser');
		global $ts;
		$huNumPerRow = 4;
		if(empty($data['hotUser'])) {
			$hotUserNum  = $huNumPerRow*10;
	    	$time_range = model('Xdata')->get('userclass:hotuser');
			...
			//获取我的班级用户ID
			$result = $this->get_class_list('uid');
			$uids = array();
			foreach($result as $r){
				array_push($uids,$r['uid']);
			}
			//将自己也加入班级列表id中
			array_push($uids,$this->mid);

			$hotUser = M()->query("SELECT uid,count(weibo_id) as weibo_num FROM {$db_prefix}weibo where ctime>{$yesterday} AND ctime<{$today} AND isdel=0 AND uid in (".implode(',',$uids).") GROUP BY uid ORDER BY weibo_num DESC LIMIT {$hotUserNum}");
	    	if($hotUser){
	    	...
	    	S('S_class_hotUser', $data['hotUser'], $cacheTime);
		}else{
        ...
        $index_type = intval( $_GET ['type'] );  //0=我的班级、1=社团微广播、2=正在发生
        $weibo_type = h( $_GET ['weibo_type'] ); //orignal=原创、0=微广播、1=图片微广播、2=视频

        $result = $this->__getWeiboList($install_app, $index_type, $weibo_type); //显示微广播列表
    ...
    private function __paramUserFollowGroup($type){
        $data ['follow_gid'] = is_numeric ( $_GET ['follow_gid'] ) ? $_GET ['follow_gid'] : 'class';
        ...
        $firstGroup =  array('follow_group_id'=>'class','title'=>L('my_class'));
		
        if($data['follow_gid'] == 'class'){
