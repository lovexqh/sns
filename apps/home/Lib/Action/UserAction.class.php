...
class UserAction extends Action {
    ...
    function index() {
        ...
        $data = $this->__getWeiboList($install_app, $index_type, $weibo_type,3); //显示微博列表
        ...
        //新功能
        $video_config = model('Xdata')->lget('video');

        //获取最新视频
        $data['videolist'] = D('Video','video')->field('*,\''.$video_config['video_server_url'].'\' as video_server_url')->order('cTime DESC')->limit('8')->select();

        //获取我的文章
        $map = array();
        $map['uid']	=	$this->mid;
        $data['myblogs'] = D('Blog','blog')->where($map)->order('cTime DESC')->limit('8')->select();

		//获取我的图片
		$map = array();
		$map['userId'] = $this->mid;;
        $data['myphotos'] = D('Photo','photo')->where($map)->order('cTime DESC')->limit('8')->select();

        //获取我的视频
		$map = array();
		$map['userId'] = $this->mid;;
        $data['myvideos'] = D('Video','video')->field('*,\''.$video_config['video_server_url'].'\' as video_server_url')->where($map)->order('cTime DESC')->limit('8')->select();

		//获取重要信息
		$announcement = model ( 'Xdata' )->where ( '`list`="announcement"' )->findAll();
		foreach ( $announcement as $v ) {
            $announce [$v ['key']] = unserialize ( $v ['value'] );
        }
        $announce ['ctime'] = strtotime ( $announcement ['0'] ['mtime'] );

        $data['announce'] = $announce;
        
        //获取应用分组
        $data['group']=model('AppGoup')->field('group_id,group_name,role_private')->where('group_id>1')->order('display_order ASC')->findAll();
        
        //获取个人相关空间数据
        $data['zoneNode'] = $this->_zoneNodeSet();
     ...
     * @author 小波
     +----------------------------------------------------------
     * 创建时间：2013-3-23 下午3:15:18
     +----------------------------------------------------------
     */
    private function _zoneNodeSet(){
    	global $ts;
    	//获取我相关的
    	$data = $_SESSION['ucInfo'];
    	$ucinfo = array();
    	foreach ($data as $key=>$value){
    		$ucinfo[strtolower($key)] = $value;
    	}
    	//如果为老师
    	if($ucinfo['identitytype'] == '2'){
    		//$result['relevant'] = uc_get_space_teacher($ucinfo['identityid']);
    	}
    	//如果为学生
    	if($ucinfo['identitytype'] == '3'){
    		$result['relevant'] = uc_get_space_student($ucinfo['identityid']);
    	}
    	
    	//获取所有学校
    	$result['xxnode'] = uc_get_all_school();
    	//获取相关年级
    	if (!empty($result['xxnode'][0])) {
    		$result['sid'] = $result['xxnode'][0][id];
    	}
    	if ( !empty($result['sid']) && intval($result['sid'])>0 ){
    		$result['njnode'] = uc_get_grades_by_sid(intval($result['sid']));
    	}
    	
    	//获取相关班级
    	if (!empty($result['xxnode'][0])) {
    		$result['gid'] = $result['njnode'][0][id];
    	}
    	if ( !empty($result['gid']) && intval($result['gid'])>0 ){
    		$result['bjnode'] = $classlist = uc_get_gradclass_by_id($result['gid'],$result['sid']);
    	}
    ...
    private function __assignTabSwitch ($index_type)
    {
        ...
        switch ($index_type) {
            ...
            case self::INDEX_TYPE_ALL:
                ...
                break;
            case self::INDEX_TYPE_DEPT:
                $this->assign('dept_tab', 'on');
                break;
            default:
                $this->assign('weibo_tab','on');
        }
    ...
    public function atme() {
    	//获取应用分组
        $data['group']=model('AppGoup')->where('group_id>1')->order('display_order ASC')->findAll();
        $this->assign ( $data );
    ...
    function collection() {
        ...
        //2013/05/29 RickerYu修改支持自动判断是否大桌面调用并调用相应组的模板 start
        if (!empty($_SESSION['system']) && $_SESSION['system'] == 'desktop') {
        	$this->display ('deskindex');
        	exit;
        }
        //2013/05/29 RickerYu修改支持自动判断是否大桌面调用并调用相应组的模板 end
        $this->display ( 'bookmark' );
    }
	
    //添加到收藏
    function add_favorite(){
    	$data['appname'] = isset($_POST['appname']) ? $_POST['appname'] : '';
    	$data['action'] = isset($_POST['action']) ? $_POST['action'] : '';
    	$data['method'] = isset($_POST['method']) ? $_POST['method'] : '';
    	$data['fid'] = isset($_POST['fid']) ? $_POST['fid'] : 0;
    	$data['appconfig'] = isset($_POST['appconfig']) ? $_POST['appconfig'] : 0;
    	if($data['fid'] != 0){
    ...
    function up_favorite(){
    	...
    	if($data['fid'] != 0){
    		$id = model('Favorite')->up_favorite($data);
    		if($id){
    			echo $id;
    			exit();
    		}
    ...
    function del_favorite(){
    	...
    	$data['appconfig'] = isset($_POST['appconfig']) ? $_POST['appconfig'] : 0;
    	if($data['fid'] != 0){
    		$id = model('Favorite')->del_favorite($data);
    		if($id){
    			echo $id;
    			exit();
    		}
    ...
    function remove_favorite(){
    	...
    	if($id != 0){
    		$data = model('Favorite')->get_favorite($id);
    		if(model('Favorite')->del_favorite($id)){
