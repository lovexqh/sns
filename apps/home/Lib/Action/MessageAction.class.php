...
class MessageAction extends Action
{
	...
	public function loadMessage()
	{
		...
		foreach ($message['data'] as $key=> $val){
			$message['data'][$key]['content'] = str_replace('&#091;', '[', $message['data'][$key]['content'] );
			$message['data'][$key]['content'] = str_replace('&#093;', ']', $message['data'][$key]['content'] );
			//$message['data'][$key]['content'] = format($message['data'][$key]['content'],true);
		}
		$this->assign('message', $message);
	...
	public function doPost() {
		$change = isset($_GET['change']) ? $_GET['change'] : false;
		if (!lockSubmit()) {
			echo -1;
			exit;
		}
	...
	*/
	...
	//审核列表
	function approved(){
		$list = X('Approved')->get('FIND_IN_SET('.$this->mid.',receive)', 10);
		// 解析表情
		foreach($list['data'] as $k => $v) {
			$list['data'][$k]['content']  = preg_replace_callback("/\[(.+?)\]/is",replaceEmot,$v['content']);
		}
		$this->assign($list);
		$this->setTitle("审核消息");
		$this->display();
	...
	//删除审核信息
	function delApproved(){
		$approved_Id = intval($_POST['id']);
		if(M('approved')->where('approved_id='.$approved_Id.' AND FIND_IN_SET('.$this->mid.',receive)')->delete()){
			echo 1;
		}
	}
	...
	function auditApproved(){
		$approved_Id = intval($_POST['id']);
		$data['audit_uid'] = $this->mid;
		$data['status'] = 1;
		$data['mtime'] = time();
		if(M('approved')->where('approved_id='.$approved_Id.' AND FIND_IN_SET('.$this->mid.',receive)')->save($data)){
			echo 1;
		}else{
			echo 0;
		}
