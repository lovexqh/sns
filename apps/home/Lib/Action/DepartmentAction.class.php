...
class DepartmentAction extends Action
{
    ...
    public function _initialize(){
		$id = intval($_GET['id']);
    	//获取部门信息
    	$dept = uc_dept_get_id($id);

    	$this->assign('dept',$dept);

    	//获取部门相关人员
    	$uc_users = uc_get_deptuser($dept['DeptID']);
    	if(!count($this->mails)){
    		$mails = array();
    		foreach($uc_users as $item){
	    		$mails[]=$item['email'];
	    	}
	    	$this->mails = $mails;
    	}
    	$field="*";
    	$map['email'] = array('in',$this->mails);
		$this->users = D("User")->field($field)->where($map)->findAll();
		if(!count($this->uids)){
    		$uids = array();
    ...
    {
		$weibos=$this-> _getweibo(10);

		$blogs=$this->_getblog();
		$photos=$this->_getphoto();
		$videos=$this->_getvideo();
		$librarys=$this->_getlibrary();
		$datas=$this->_hebing($blogs,$photos,$videos,$librarys);
		foreach ($datas as $key => $row) {
            $volume[$key] = $row['type'];
        }
		$times=$this->_dates($datas);
		$types=$this->_types($datas);
    ...
    public function _getblog()
    {
		...
		$map['uid']=array('in',$this->uids);
		$map['status']='1';
		$blogs=$blog->where($map)->field("id,uid,title,content,cTime,(select '文章') as typename,(select 'blog') as type")->order('cTime DESC')->limit(10)->select();
    ...
    {
		$photo=D('Photo');
		$map['userId']=array('in',$this->uids);
		$photos=$photo->where($map)->field("id,userId as uid,name,cTime,savepath,(select '照片') as typename,(select 'photo') as type")->order('cTime DESC')->limit(10)->select();
    ...
    public function _getvideo()
    {
		...
		$map['userId']=array('in',$this->uids);
		$videos=$video->where($map)->field("id,userId as uid,name,cTime,(select '$serverurl') as serverurl,savepath,fcode,(select '视频') as typename,(select 'video') as type")->order('cTime DESC')->limit(10)->select();
		$videos=$this->replace($videos);
		return $videos;
    }

    //获取文库信息
    public function _getlibrary()
    {
		$library=D('Library');
		$map['userId']=array('in',$this->uids);
		$librarys=$library->where($map)->field("id,userId as uid,name,cTime,savepath,(select '文库') as typename,(select 'library') as type")->order('cTime DESC')->limit(10)->select();
		$librarys=$this->replace($librarys);
		return $librarys;
    }

    //合并数据
    private function _hebing($blogs=array(),$weibos=array(),$photos=array(),$videos=array(),$librarys=array())
    {
		$datas=array_merge($blogs,$weibos,$photos,$videos,$librarys);
		return $datas;
    }
	//获取日期
    ...
    private function _createObj($arr){
		...
		foreach($arr as $event){
			$type = $event['typename'];
			$events[$type][] = $this->_eventObj($event);
		}
		return $events;
	}
	private function _eventObj($event){
		if(is_array($event)){
			...
			if(!empty($event['serverurl']))$ev['serverurl'] = $event['serverurl'];
			if(!empty($event['fcode']))$ev['fcode'] = $event['fcode'];
		}
		else{
    ...
    private function replace($data){
    	...
    	foreach($result as &$value){
		   if($value['content']){
			   	$value['content'] = wsubstr(strip_tags(htmlspecialchars_decode($value['content']),'<img>'),255);
		   }else{
				$value['content'] = '';
			}
		   if($value['savepath']){
