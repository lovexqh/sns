...
class SelectFriendsAction extends Action {
		...
		public function getClassOne() {
				...
				$userlist=uc_student_get_id_byname($classid,$name);
			    $userids=array();
			    $n=0;
				...
				if($name){
					//从我关注的人中找				
					$followings = $Model->field('snsuser.uid AS fuid,snsuser.uname AS funame')
				...
				//合并，并过滤重复
				is_array($followings) || $followings=array();//查询返回空时，不为数组，则需转为空数组
				$follow     = $this->unique_arr($followings);
                foreach($follow as $k=>$v) {
                        $out[$k]['fUid'] = $v['0'];
                        $out[$k]['friendUserName'] = $v['1'];
                        $out[$k]['friendHeadPic'] = getUserFace($v['0']);
        ...
        public function getClassAll() {
				...
				$uids=implode(",",$userids);
				$db_prefix  =  C('DB_PREFIX');
				$follow=M('')->field('snsuser.uid AS fuid,snsuser.uname AS funame')
						   ->table("{$db_prefix}user AS snsuser LEFT JOIN {$db_prefix}ucenter_user_link AS uc_user ON snsuser.uid=uc_user.uid")
						   ->where("uc_user.uc_uid in ($uids)")
						   ->order('snsuser.uid ASC')
						   ->findPage(15); 
        ...
        public function getClassType() {
                ...
                //$friendType = D("FriendGroup")->where($map)->field("id,name")->findAll();
				$typeId = array(
							    array('id'=>1,'name'=>L('follow_class')),
							  );
                echo json_encode( $typeId );
        }
		public function getClassCount() {
				...
				$uids=implode(",",$userids);
				$db_prefix  =  C('DB_PREFIX');
				$followNum = M('')->table("{$db_prefix}user AS snsuser LEFT JOIN {$db_prefix}ucenter_user_link AS uc_user ON snsuser.uid=uc_user.uid")
						   ->where("uc_user.uc_uid in ($uids)")
						   ->order('snsuser.uid ASC')->count();
				echo $followNum;
        }
		//----------------------------------------------班级选择同班同学  end--------------------------------
