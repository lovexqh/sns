...
function desktopCheck(form){
	$(form).form('submit', {
		...
		success: function(data){
			if(data == 1){
				parent.tabs.close('我发起的活动');
				parent.tabs.iframe("我发起的活动",U('event/Index/personal',['action=launch']));
				parent.tabs.close('创建活动');
			}else if(data == 0){
				$.messager.alert('发起活动','发起活动失败！'); 
			}else{
				$.messager.alert('发起活动失败',data);
			}
