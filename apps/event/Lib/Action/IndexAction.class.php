...
class IndexAction extends Action {
    ...
    public function index() {
        ...
        switch( $_GET['order'] ) {
        	case 'new':    //最新排行
                ...
                break;
            case 'hot':    //最新排行
                $order = 'cTime DESC,joinCount DESC,attentionCount DESC';
                $this->setTitle('最新' . $this->appName);
                break;
        ...
        //$title = t($_POST['title']);
        if ($data['search_key']) {
        	$map['title'] = array( 'like',"%".t($data['search_key'])."%" );
    ...
    public function addEvent() {
        ...
        if (!empty($_SESSION['system']) && $_SESSION['system'] == 'desktop') {
        	 $re = $this->_desktopcreateLimit($this->mid);
        }else{
    ...
    private function _desktopcreateLimit($uid){
    	$config = getConfig();
    
    	if(!$config['canCreate']){
    		return '禁止发起'.$this->appName;
    	}
    	if($config['credit']){
    		$userCredit = X('Credit')->getUserCredit($uid);
    		if($userCredit[$config['credit_type']]['credit']<$config['credit']){
    			return $userCredit[$config['credit_type']]['alias'].'小于'.$config['credit'].'，不允许发起'.$this->appName;
    		}
    	}
    	if( $timeLimit = $config['limittime'] ){
    		$regTime = M('user')->getField('ctime',"uid={$uid}");
    		...
    		if($difference<$timeLimit){
    			return '账户创建时间小于'.$timeLimit.'小时，不允许发起'.$this->appName;
    		}
    	}
    	return 1;
    }
    ...
    public function ajaxAddEvent() {
    	...
    	$allow               = isset( $_POST['allow'] )?1:0;
    	$opts['opts']        = array( 'friend'=>$friend,'allow'=>$allow );
    	$rs = $this->event->doAddEvent( $map,$opts,$cover );
    	if( $rs ) {
    		X('Credit')->setUserCredit($this->mid,'add_event');
    		echo 1;
    	}else{
    ...
    public function edit(  ) {
        ...
        if( $uid != $this->mid ) {
            $this->error( '您没有权限编辑'.$this->appName ) ;
        }
     ...
     * @return void
     */
    public function doajaxEditEvent() {
    	$id['id'] = intval( $_POST['id'] );
    	$id['optsId'] = intval( $_POST['optsId'] );
    	$map['title']      = t($_POST['title']);
    	$map['address']    = t($_POST['address']);
    ...
    public function doAgreeAction() {
        ...
        //检查操作权限
        if( $this->mid != intval(D('Event')->getField('uid',"id={$data['eventId']}")) ){
            echo  -4;
            return;
        }
        if($result['lc']!='无限制'&& intval($result['lc'])<=0){
            echo -5;
            return ;
        }
        //检测id和uid是否为0
    ...
    public function checkUrl(array $data ) {
  	...
  	public function favorite(){
  		$result  = model('Favorite')->list_favorite_all('event',20,'event');
  		$result = $this->event->getEventListByResult($result);
  		$message='暂时没有收藏的活动';
  		$this->assign($result);
  		$this->assign("message",$message);
