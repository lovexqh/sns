...
class EventModel extends BaseModel{
	...
	function getEventListByResult($result){
		//追加必须的信息
		if( !empty( $result['data'] )){
			$user = self::factoryModel( 'user' );
			//$friendsId = $this->api->friend_get();
	...
	public function appendContent( $data ){
		...
		$data['province']    = $opts_list['province'];
		$data['subcity']    = $opts_list['city'];
		$data['area']    = $opts_list['area'];
	...
	public function doAddEvent($eventMap,$optsMap,$cover){
		...
		if( 1 == $has_friend  ){
			//我关注的人的ID
			$fids = M('weibo_follow')->field('fid')->where("uid={$eventMap['uid']} AND type='following'")->findAll();
			foreach($fids as $k=>&$v){
				$fids[$k] = $v['fid'];
			}
				...
				switch( $map['action'] ){
					...
					case 'joinIn':
						if($event_user['status']){
	                        ...
	                        $this->setDec( $delete,$deleteMap );
                            X('Credit')->setUserCredit($data['uid'],'cancel_join_event');
						}
						break;
				}
				return 1;
