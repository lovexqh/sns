...
class CommentModel extends Model {
    ...
    public function doaddcomment($uid, $post, $api=false)
    {
        ...
        if ( $comment_id = $this->addcomment( $data ) ){
            ...
            if( $post['transpond'] != 0 ){
			    ...
			    if ($id) {  //当转发的微广播uid 与 回复人的uid不一致时发布@到我
    ...
    function getCommentList($type='receive',$person='all',$uid) {
    	...
    	// 缓存被评论的微广播, 被回复的评论, 评论的发表人, 被回复的用户
     ...
     * 缓存微广播评论
     *
     * 缓存的key的格式为: weibo_comment_微广播评论ID.
     *
     * @param array $comment_list 微广播评论ID列表, 或者微广播评论详情列表. 如果为微广播评论ID列表时, 本方法会首先获取微广播评论详情列表, 然后缓存.
     */
