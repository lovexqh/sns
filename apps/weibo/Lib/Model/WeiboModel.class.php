...
class WeiboModel extends Model{
    ...
    protected function getWeiboDetail($id,$simple=0){
        if(is_array($id)){
    ...
    function transpond($uid,$data,$api=false){
        ...
        if( $data['reply_weibo_id'] ){ //对相应微广播ID作出评论
    ...
    private function __parseTemplate( $value ){
        ...
        Addons::hook('weibo_type_parse_tpl',array('typeId'=>$type,'typeData'=>$typedata,'rand'=>$rand,'simple'=>$simple,'result'=>&$template));
        return $template;
    }
     ...
     * 缓存微广播列表
     *
     * 缓存的key的格式为: weibo_微广播ID.
     *
     * @param array $weibo_list 微广播ID列表, 或者微广播详情列表. 如果为微广播ID列表时, 本方法会首先获取微广播详情列表, 然后缓存.
     */
    ...
    //微广播人气排行榜 - 按一段时间(默认30天)内 微广播被转发和评论的权重计算 默认 转发数*5+评论数*5
