...
class TopicModel extends Model{
	...
	public function getHot($type=null, $range=null,$limit=10)
	{
		...
		foreach($hot_list as $k=>$v){
			$hot_list[$k]['count'] = M('weibo')->field('weibo_id')->where("`weibo_id` IN (SELECT weibo_id FROM ".C('DB_PREFIX')."weibo_topic_link WHERE topic_id = ".$v['topic_id'].") ")->count();
