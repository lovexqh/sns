...
class WeiboApiModel extends WeiboModel
{
    ...
    //获取最新更新的公共微广播消息
    public function public_timeline($since_id, $max_id,$limit)
    {
//		$limit = ($page-1)*$count.','.$count;
        $map['type'] = array('IN',array(0,1,999,3));
        if($since_id){
    ...
    //获取当前用户所关注用户的最新微广播信息
    public function friends_timeline($uid, $since_id, $max_id,$limit)
    {
...
//		$map['type'] = array('IN', array('1','0'));
        ...
        if($followCount)
            $map['uid']   = array('exp', "IN ( {$now_following_sql} )");
    ...
    //获取当前用户所关注用户的最新微广播信息
    public function class_timeline($cid, $since_id, $max_id,$limit)
    {
...
//		$map['type'] = array('IN', array('1','0'));
    ...
    //获取用户发布的微广播信息列表
    public function user_timeline($uid, $uname, $since_id, $max_id, $limit)
    {
        if (!$uid) {
...
//		$map['type'] = array('IN',array('1','0'));
    ...
    //获取@当前用户的微广播列表
    public function mentions($uid, $since_id, $max_id, $limit)
    {
    ...
    //获取API评论
    function comments($id,$since_id,$max_id,$limit){
        if($since_id){
            $map['comment_id'] = array('gt',$since_id) ;
        }elseif ($max_id){
            $map['comment_id'] = array('lt',$max_id);
    ...
    //获取用户关注的列表
    function following($uid,$uname,$since_id,$max_id,$limit){
        if(!$uid){
            $user = M('user')->where("uname='$uname'")->field('uid')->find();
            $uid = $user['uid'];
        }
...
//            $list[$k]['weibo'] = $this->getOneApi('', $mini);
    ...
    //获取用户被关注列表
    function followers($uid,$uname,$since_id,$max_id,$limit){
        if(!$uid){
            $user = M('user')->where("uname='$uname'")->field('uid')->find();
            $uid = $user['uid'];
        }
...
//            $list[$k]['weibo'] = $this->getOneApi('', $mini);
    ...
    //搜索用户
    function searchUser($key,$mid,$since_id,$max_id,$limit){
        $key=t($key);
        if(!$key) return false;
//        $map = 'uid>0';
//        if($since_id){
...
//            $map.= " AND uid < $max_id";
//        }
        $list = $this->table(C('DB_PREFIX').'user')->where("uid<>$mid AND uname LIKE '%{$key}%'")->field('uid,uname,sex,face')->limit($limit)->findall();
        foreach ($list as $k=>$v){
//            $list[$k]['mini'] = M('weibo')->where('uid='.$v['uid'].' AND type=0')->order('weibo_id DESC')->find();
            ...
            $list[$k]['weibo_count']	= $this->where('uid='.$v['uid'])->count();
