    ...
    weibo:function(setting){
        ...
        $.extend(this,{
                ...
                favorite:function(id,o){
                    ...
                    if($(o).attr('onclick')==null) return;
                    var _click=$(o).attr('onclick');
                           ...
                           // $('#content_'+id).html('已收藏');
                            ...
                            setTimeout(function(){
                            	$('#content_'+id).fadeOut('fast',function(){
                            		$('#content_'+id).html('<a onclick="weibo.delFavorite('+id+',this)" href="javascript:void(0)">取消收藏</a>').fadeIn('fast');
                            	});
                            },500);
                        }else{
                            ui.error('收藏失败');
        					$(o).attr('onclick', _click);
							$(o).removeAttr('disabled');
                        }
                    },{id:id});
                },
				//实时删除微博中收藏
                delFavorite:function(id,o){
                    if(_MID_ <= 0){
                        ui.error('请登录后再操作'); return ;
                    }
					if($(o).attr('onclick')==null) return;
                    ...
                    operateFactory.create('unFavorite',function(txt){
                         if( txt ){
							$(o).wrap('<span id=content_'+id+'></span>');
                            //$('#content_'+id).html('已取消');
							//替换文字
							setTimeout(function(){
								$('#content_'+id).fadeOut('fast',function(){
										$('#content_'+id).html('<a onclick="weibo.favorite('+id+',this)" href="javascript:void(0)">收藏</a>').fadeIn('fast');
									});
							},500);
                        }else{
                            ui.error('取消失败');
