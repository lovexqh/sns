<?php
/**
 * 根据社团Logo的保存路径获取Logo的URL
 *
 * @param string $save_path 社团Logo的保存路径
 * @return string 社团Logo的URL. 给定路径不存在时, 返回默认的社团Logo的URL地址.
 */
 ...
 * 根据社团ID获取社团详情页的URL
 *
 * @param int $group_id 社团ID
 * @return string 社团详情页的URL
 */
...
/*
 * RickerYu 2013-5-9 13:46重写
 * 判断是管理者或创建者都可以修改
 */
function iscreater($uid,$gid) {
	return D('Member')->where("uid=$uid AND gid=$gid AND level IN (1,2)")->count();
}
...
function getSiteTitle(){
	return array(
		'my_group'=>'所在社团的最新动态-我的社团-',
		'my_group_new_topic' => '最新话题-我的社团-',
		'my_friend_group' => '好友的社团-',
		'all_group' => '发现社团-',
