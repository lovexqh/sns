...
class ToolAction extends AdministratorAction {
	...
	/**
	 ...
	 +----------------------------------------------------------
	 + 创建时间：	2013-9-23 上午10:01:06
	 +----------------------------------------------------------
	 */
	public function dbReplace(){
	    $this->assign('oldurl',getSiteUrl());
	    $this->display();
	}
	/**
	 +----------------------------------------------------------
	 ...
	 + @author	小波 (Administrator)
	 +----------------------------------------------------------
	 + 创建时间：	2013-9-23 上午8:25:35
	 +----------------------------------------------------------
	 */
	public function doDbReplace(){
	    $local = $_SERVER['REQUEST_SCHEME']."://".$_SERVER['REMOTE_ADDR'];
	    //要替换前的域名地址
	    $str_s = $_POST['oldurl']?$_POST['oldurl']:$local;
	    //要替换后的域名地址
	    $str_r = $_POST['newurl']?$_POST['newurl']:$local;
	    //$str_s = $str_r = "http://localhost";
	    //$str_r = "http://127.0.0.1";
	    //统计区修改了几个表的变量
	    $tabcols = 0;
	    $cols = 0;
	    $tmptab = '';
	    ...
	    //$link = mysql_connect(C('DB_HOST').':'.C('DB_PORT'), C('DB_USER'), C('DB_PWD'));
	    foreach($tables as $table) {
	        $T = $table['Tables_in_'.C('DB_NAME')];
	        //2、获取所有列数据
    		$fields = mysql_list_fields(C('DB_NAME'), $T);
    		if( strpos($T, "ts_") === 0 ) $T = substr($T, 3);
    		$columns = mysql_num_fields($fields);
    		for ($i = 0; $i < $columns; $i++) {
    			$field = mysql_field_name($fields, $i);
    			$data = M($T)->where($field . " like '%" . $str_s . "%'")->findAll();
    			if( $data ) {
    				foreach($data as $vo) {
    					$field_new = $vo[$field];
    					preg_match_all("/(s:[\d]+:\"[^\"]*http:\/\/[^\"]*\")/i", $vo[$field], $arr2);
    					// $field_new 替换前的数据
    					if($arr2[0]) {    //开始替换
    						foreach($arr2[0] as $str2) {
    							$p = strpos($str2, ":\"");
    							$str3 = substr( $str2, $p + 1 );
    							$str3 = str_replace($str_s, $str_r, $str3);
    							$str2_new = "s:" . (strlen($str3) - 2) . ":" . $str3;
    							$field_new = str_replace($str2, $str2_new, $field_new);
    							// 替换前的数据echo $str2 . "<br />";
    							// 替换后的数据echo $str2_new . "<br />";
    						}
    					} else {    //非序列化数据直接处理
    						$field_new = str_replace($str_s, $str_r, $field_new);
    					}
    					//如果有替换操作时
    					if(strcasecmp($vo[$field],$field_new)!=0){
    					    $map = $vo;
							foreach($map as $k=>$v){
								if(empty($v)) unset($map[$k]);
							}
    					    unset($map[$field]);
    					    $r = M($T)->where($map)->save(array($field=>$field_new));
    					    if($r){
    					        $cols++;
    					        if($tmptab == $T){
    					            $tmpnum++;
    					            ...
    					            echo "<div style='border:2px solid green; background:#f1f1f1; padding:20px;margin:20px;width:800px;font-weight:bold;color:green;text-align:center;'> {$tmptab}表中{$tmpnum}条数据被替换! </div>";
    					            $tmpnum=0;
    					            $tmptab = $T;
    					        }
    					    }
    	...
    	echo "<div style='border:2px solid red; background:#f1f1f1; padding:20px;margin:20px;width:800px;font-weight:bold;color:red;text-align:center;'> 共修改了{$tabcols}个表中的{$cols}条数据. </div> <br />";
