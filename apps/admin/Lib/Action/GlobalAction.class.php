...
class GlobalAction extends AdministratorAction {
    ...
    public function doSetSiteOpt() {
        ...
        if (intval($_POST['length']) <= 0) {
            $this->error('全站微广播、评论字数限制的值必须大于0');
        }
    ...
    public function doEditCreditType(){
        ...
        //错误信息
        $error = $creditTypeDao->getDbError();
        $_LOG['uid'] = $this->mid;
        $_LOG['type'] = '3';
        $data[] = '全局 - 积分配置 - 积分类型 ';
        $data[] = $oldName;
    ...
    public function doEditCredit() {
        ...
        //错误信息
        $error = M('credit_setting')->getDbError();
    ...
    public function doCreditUser(){
        ...
        if($act == 'set'){//积分修改为
            foreach($user as $v){
                ...
                if($setCredit->getInfo()===false)$this->error('设定失败');
            }
        }else{//增减积分
            foreach($user as $v){
                $setCredit->setUserCredit($v['uid'],$action);
                if($setCredit->getInfo()===false)$this->error('增减失败');
            }
        }
