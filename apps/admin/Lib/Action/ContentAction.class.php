...
class ContentAction extends AdministratorAction {
	...
	public function editAd() {
		...
		//$this->assign('ad',$ad);
        //edit@Snail 20131230 反编译内容
		$ad_content = stripslashes($ad['content']);
		if($ad_content){
...
//编辑广告
	...
	public function annocontent() {
		$limit=20;
		$data = M('annocontent')->order('`ctime` desc,`content_id` desc')->findAll();
		//$data = M('annocontent')->order('`ctime` desc')->findPage($limit);
		//dump($data);
		$this->assign('annocontent', $data);
		$this->display();
	}

	public function addAnno() {
		$this->assign('type', 'add');
		$this->display('addAnno');
	}

	public function editAnno() {
		$map['content_id'] = intval($_GET['content_id']);
		$annocontent = M('annocontent')->where($map)->find();
		$this->assign('annocontent',$annocontent);
		$this->assign('type', 'edit');
		$this->display('editAnno');
	}

	public function doUpdateAnno(){
		$map['content_id'] = $_POST['content_id'];
		$data = M('annocontent')->where($map)->find();	
		// 格式化数据
        $data['content']= h(t($_POST['content']));
	    $data['grade']	= intval($_POST['grade']);
		$data['mtime']	= time();
		// 提交数据
		$res = isset($_POST['content_id']) ? M()->table(C('DB_PREFIX').'annocontent')->data($data)->where($map)->save() : M()->table(C('DB_PREFIX').'annocontent')->add($data);

		if($res) {
			if( !isset($_POST['content_id']) ) {

				$this->assign('jumpUrl', U('admin/Content/addAnno'));
			}else {
				$this->assign('jumpUrl', U('admin/Content/annocontent'));
			}
			F('_action_annocontent',null);	//删除公告缓存
			$this->success('保存成功');
		}else {
			$this->error('保存失败');
		}
...
//编辑公告
	public function updateAnno(){
		$map['content_id'] = intval($_GET['content_id']);
		$annocontent = M('annocontent')->where($map)->find();
		$this->assign('annocontent',$annocontent);

		$content = unserialize($annocontent['content']);
		if($content){
			$annocontent['content'] = $content;
		}
		if(empty($annocontent))
			$this->error('参数错误');
			$this->assign($annocontent);
			$this->assign('type', 'edit');
			$this->display('updateAnno');
	}
	public function doEditAnno() {
		
		if( ($_POST['content_id'] = intval($_POST['content_id'])) <= 0 )
		...
		if(empty($_POST['content']))
			$this->error('内容不能为空');
		if($_POST['grade'] < 0 || $_POST['grade'] > 1)
			$this->error('参数错误');

		$_LOG['uid'] = $this->mid;
		$data[] = '内容 - 公告管理 ';
		isset($_POST['content_id']) && $data[] =  M('annocontent')->where( array( 'content_id'=>intval($_POST['content_id']) ) )->find();
		if ( isset($_POST['content_id']) ) unset( $data['1']['ctime'] );
		if( $_POST['__hash__'] )unset( $_POST['__hash__'] );
		$data[] = $_POST;
		$_LOG['data'] = serialize($data);
		$_LOG['ctime'] = time();
		...
		$res = isset($_POST['content_id']) ? M()->table(C('DB_PREFIX').'annocontent')->save($_POST) : M()->table(C('DB_PREFIX').'annocontent')->add($_POST);

		//编辑控制
		$aid = $_GET['id'];
		$editdata = M( 'annocontent' )->where( 'content_id='.$aid )->find();
		$this->assign('editdata',$editdata);
	...
	public function doDeleteAnno() {
		if( empty($_POST['ids']) ) {
			echo 0;
			exit ;
		}
		$map['content_id'] = array('in', t($_POST['ids']));

		$_LOG['uid'] = $this->mid;
		$_LOG['type'] = '2';
		$data[] = '内容 - 公告管理 ';
		$data[] =  M('annocontent')->where( $map )->findall();
		$_LOG['data'] = serialize($data);
		$_LOG['ctime'] = time();
		M('AdminLog')->add($_LOG);

		echo M('annocontent')->where($map)->delete() ? '1' : '0';
		F('_action_annocontent',null);	//删除公告缓存
	}
