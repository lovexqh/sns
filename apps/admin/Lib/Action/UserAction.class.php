...
class UserAction extends AdministratorAction {
	...
	public function setField() {
		$utypeid = intval($_REQUEST['utypeid']);
		$data['list'] = D('UserSet')->getFieldList($utypeid);
		$data['user_groups'] = model('UserGroup')->getUserGroupByMap('isreg>=0');
		foreach ($data['user_groups'] as $r){
			$data['utype'][$r['user_group_id']] = $r['title'];
		}
		$this->assign( $data );
		$this->display();
	}
	...
	public function editField(){
		...
		$user_groups = model('UserGroup')->getUserGroupByMap('isreg>=0');
	...
	public function userGroup() {
		/*if(UC_SYNC){	//获取UCenter的用户角色数据
		 $result = uc_identity_list();
		 $user_groups = array();
		 foreach ($result as $r) {
		 $item = array();
		 $item["user_group_id"]	= $r["IdentityID"];
		 $item["title"]			= $r["IdentityName"];
		 $item["isreg"]			= $r["isreg"];
		 $item["ctime"]			= "";
		 $item["icon"]			= intval($r["IdentityID"])==1?"v_05.gif":UC_API."/".$r["IdentityIcon"];
		 $user_groups[] = $item;
		 }
		 }else*/
	...
	public function doAddUserGroup() {
		...
		if ( $dao->isUserGroupExist($_POST['title']) ) {
			...
			$group['isreg']		= $_POST['isreg'];
			$group['icon']		= $_POST['icon'];
			$res = $dao->addUserGroup($group);
	...
	//同步用户组数据
	public function doSyncUserGroup(){
		if(UC_SYNC){
			$result = uc_identity_list();
			if(empty($result)){
				echo 0;
			...
			//清空TS表数据
			M('UserGroup')->delete(array('exp', 'LIKE "*"'));
			foreach ($result as $r) {
				$data = array();
				$data["user_group_id"]	= $r["identityid"];
				$data["title"]			= $r["identityname"];
				$data["isreg"]			= $r["isreg"];
				$data["ctime"]			= time();
				$data["order"]			= $r["order"];
				$data["icon"]			= intval($r["identityid"])==1?"v_05.gif":basename($this->__grabImage(UC_API."/".$r["identityicon"],SITE_PATH."/public/themes/edustyle/member/images/usergroup/".basename($r["identityicon"])));
	...
	//获取一张图片
	private function __grabImage($url,$filename="") {
		if($url=="") return false;
	...
	public function classes() {
		...
		if($page){
			$dataList = M('UserClass')->order( 'class_name ASC')->limit( '0,'.$limit )->findAll();
		}else{
			$dataList = M('UserClass')->order( 'class_name ASC' )->findPage($limit);
		}

		$this->assign('html',$dataList['html']);
		$this->assign('user_classes',$dataList['data']);
		$this->display();
	}
	...
	public function editUserClass() {
		...
		if ($id>0) {
			//编辑时，显示班级信息
			$user_class = M('UserClass')->where("class_id = $id")->find();
			$this->assign('user_class', $user_class);
		}
		$this->display();
	}

	public function doAddUserClass() {
		$grade = $_POST['class_grade'];
		$_POST['class_name'] = escape(t($_POST['class_name']));
		...
		$dao = model('UserClass');
		if ( $dao->isUserClassExist($_POST['class_name']) ) {
			echo -1; // 用户班级已存在
		}else{
			$_LOG['uid'] = $this->mid;
			$_LOG['type'] = '1';
			...
			$class['class_grade']		= $_POST['class_grade'];
			$res = $dao->addUserClass($class);
			if($res) echo intval($res);
			else	 echo 0;
		}
	}

	public function doEditUserClass() {
		$id = intval($_POST['class_id']);
		$dao = model('UserClass');
		$data['class_name'] = escape(t($_POST['class_name']));
		$data['class_grade']  = escape(t($_POST['class_grade']));

		if ( $dao->isUserClassExist($data['class_name']) ) {
			echo -1; // 用户班级已存在
		}else {
			$_LOG['uid'] = $this->mid;
			$_LOG['type'] = '3';
			$data[] = '用户 - 用户班级管理 ';
			$data[] = M('UserClass')->where('class_id='.$id)->data($data)->findAll();
			if( $_POST['__hash__'] )unset( $_POST['__hash__'] );
			$data[] = $_POST;
			$_LOG['data'] = serialize($data);
			$_LOG['ctime'] = time();
			...
			$res = M('UserClass')->where('class_id='.$id)->data($data)->save();
			$res1 = M('user_class_link')->where('user_class_id='.$id)->setField('user_class_name', $data['class_name']) && $res;
			if(false !== $res) echo 1;
			else     echo 0;
		}
	}

	public function doDeleteUserClass() {
		$_POST['class_id'] = t($_POST['class_id']);
		//不为空时，不允许删除
		if ( ! model('UserClass')->isUserClassEmpty($_POST['class_id']) ) {
			echo 0;
			return ;
		}
		//提交删除
		...
		$_LOG['type'] = '2';
		$data[] = '用户 - 用户班级管理';
		$data[] = M('UserClass')->where('user_class_id='.$_POST['class_id'])->find();
		if( $_POST['__hash__'] )unset( $_POST['__hash__'] );
		$data[] = $_POST;
		$_LOG['data'] = serialize($data);
		$_LOG['ctime'] = time();
		...
		$res = model('UserClass')->deleteUserClass( $_POST['class_id'] );
		if($res) echo 1;
		else     echo 0;
	}

	public function isUserClassExist() {
		$res = model('UserClass')->isUserClassExist( $_POST['class_name'], intval($_POST['class_id']));
		if($res) echo 1;
		else	 echo 0;
	}

	public function isUserClassEmpty() {
		$res = model('UserClass')->isUserClassEmpty( $_POST['class_id'] );
		if($res) echo 1;
		else	 echo 0;
	}
}