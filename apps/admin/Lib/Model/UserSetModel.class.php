...
class UserSetModel extends Model{
    ...
    function getFieldList($utypeid=0){
    	if($utypeid>0){
    		$map['utypeid'] = array('in',array(0,$utypeid));
    	}
        return $this->table(C('DB_PREFIX').'user_set')->where($map)->order('utypeid ASC')->findall();
    }
    ...
    function addfield(){
        ...
        if( $this->table(C('DB_PREFIX').'user_set')->where($data)->count()==0 ){
            ...
            $data['utypeid']	  =  intval($_POST['utypeid']);
