...
function getUserFace($uid,$size='m'){
	...
	if(is_file($userface)){
		...
		return THEME_URL."/images/member/user_pic_$type.gif";
	}
}
...
function getUserGroupIcon($uid){
	...
	if($groupIcon){
		foreach ($groupIcon as $v){
    		if($v['icon']){
    			$html.="<img class='ts_icon' src=".THEME_URL."/member/images/".$v['icon']." title=".$v['title'].">";
    		}
    	}
    	return $html;
   	}else{
...
function getUserGroup($uid){
	$prefix = getDbPrefix();
	$sql = "SELECT * FROM {$prefix}user_group WHERE `user_group_id` IN (SELECT `user_group_id` FROM {$prefix}user_group_link WHERE `uid` = $uid)";
    $group = doQuery($sql);
...
function getUserVerifiedIcon($uid)
{
    ...
    if ($verified[0]) {
    	$html = "<img class='ts_icon' src=" . THEME_URL."/member/images/v_01.gif title=" . $verified[0]['info'] . ">";
	}
	return $html;
}
