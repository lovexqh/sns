...
//关注的人	
	public function friline(){
		...
		$conditon['type']=0;
	    $following = M('weibo_follow')->field('fid')->where($conditon)->findPage(15);
		$uids=array();
		$comfol=array();
		...
		$this->assign('html',$following['html']);
		$this->assign('userinfo',$userinfo);
        $this->display();
...
//头部信息
	private function _linehead($map){
		...
		if($userinfo['location']==null) $userinfo['location']='未填写';
		$map['type']=0;
...
//获取微广播
    public function _getweibo($limit=NULL)
    {
		...
		$map['uid']=$this->uid;
		$map['isdel']=0;
		$weibos=$weibo->where($map)->field('weibo_id,content,ctime')->order('ctime desc')->limit($limit)->select();
