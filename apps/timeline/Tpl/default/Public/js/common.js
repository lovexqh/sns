...
function addblog(){
	ui.box.load(U('blog/Index/addAjaxBlog'),{title:'写文章'});
}

function eblog(id,mid){
	var id = id ;
	window.open(U('blog/Index/edit')+'&id='+id+'&mid='+mid,'编辑文章');
}

function deleteBlog( id ){
	if(confirm("是否确定删除这条文章")) {
		$.post(U('blog/Index/doDeleteblog'),{id:id},function(data){
				$('#'+'r'+id+'bl').remove();
				ui.success('删除成功！');
                resize();
		});
