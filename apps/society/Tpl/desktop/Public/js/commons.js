 ...
 * 
 * @param status
 *            error|info|question|warning|default
 */
 ...
 * 
 * @param options
 *            title:进度条窗口的标题, msg:进度条上面提示的文字, text:进度条上的文字,
 ...
 * 
 * @param options
 *            title:无title属性的时候会显示默认值，如果title：null那么将不会显示标题,
 *            msg:消息模块中要显示的信息，支持HTML代码, width:窗口宽度, height:窗口高度,码,
 *            timeout:消息窗口自动关闭的时间，设置为0时则不会自动关闭, style:自定义的展示样式（前期不建议使用）
 */
show:function(options){
if('undefined' == typeof(options.title)){
 ...
 * 
 * @param id:要收起的元素父级ID
 *            status:要收起的方式 status的值 north, south, east, west
 */
...
// var refresh_tab = title ? $('#tabs-main').tabs('getTab',title) :
// $('#tabs-main').tabs('getSelected');
// if(refresh_tab){
// if(refresh_tab.find('iframe').length == 0) {
// url = url ? url : refresh_tab.attr("url");
// url = url ? url : window.location.href;
// url = url ? url + (url.indexOf('&') != -1 || url.indexOf('?') != -1 ? '&' :
// '?') + 'iframe=yes&' + Math.random() * 100 : '';
// refresh_tab.html('<iframe scrolling="no" onLoad="iframe_height(this);"
...
if(refresh_tab.find('iframe').length == 0) {
/*
 * url = url ? url : refresh_tab.attr("url"); url = url ? url :
 * window.location.href; url = url ? url + (url.indexOf('&') != -1 ||
 * url.indexOf('?') != -1 ? '&' : '?') + 'iframe=yes&' + Math.random() * 100 :
 * ''; refresh_tab.html('<iframe scrolling="no" onLoad="iframe_height(this);"
 * frameborder="0" src="' + url + '" style="width:100%;height:100%;"></iframe>');
 */
 ...
 * 
 * @param option
 *            option的配置 maskMsg string 提示信息 加载…… zIndex number zIndex值 100000
 *            timeout number 超时(毫秒) 30000 opacity number 透明程度.(0-1) 0.6
 */
...
var $target = $(target);
// $target.html('<img src="'+_THEME_+'/images/icon_waiting.gif" width="20"
// style="margin:10px 50%;" />');
}
...
/**
 * http://www.openjs.com/scripts/events/keyboard_shortcuts/ Version : 1.00.A By
 * Binny V A 键盘绑定事件 License : BSD
 */
...
/**
 * 对_select控件后面显示错误的那种信息 信息内容在控件是_select.text,在显示之前定义一下 _select = $('#cid0');
 * _select.text = '请输入分类信息'; showInfo(_select);
 * 
 */
...
/**
 * 用于展开、隐藏图层 id 为所要显示隐藏的图层 e 为点击事件
 */
...
// var html = '<li onmouseover="$(this).find(\'.del\').show()"
// onmouseout="$(this).find(\'.del\').hide()"><a class="del right" title="删除"
// href="javascript:void(0)"
// onclick="deleteFollowTopic(this,\''+txt.topicId+'\')"></a><a
// href="'+U('home/user/search',['k='+txt.name])+'">'+txt.name+'</a></li>';
